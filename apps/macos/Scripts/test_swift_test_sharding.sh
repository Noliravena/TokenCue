...
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tokencue-test-sharding.XXXXXX")"
...
if [[ "$*" == "test list" ]]; then
  ...
  printf '%s\n' \
    "TokenCueTests.Alpha/test_one()" \
    "TokenCueTests.Alpha/test_two(argument:)" \
    "TokenCueTests.Beta/test_two" \
    "TokenCueTests.Gamma/test_three" \
    "TokenCueTests.Delta/test_four" \
    "TokenCueTests.Epsilon/test_five" \
    "TokenCueTests.Zeta/test_six" \
    "TokenCueTests.Eta/test_seven" \
    "TokenCueTests.Theta/test_eight" \
    'TokenCueTests.`top level works`()' \
    'TokenCueTests.`top/level slash works`()'
  exit 0
fi
...
if "TOKENCUE_TEST_SHARD_INDEX=${{ matrix.shard-index }}" not in job:
    raise SystemExit("swift-test-macos must pass matrix.shard-index to Scripts/test.sh")
if "TOKENCUE_TEST_SHARD_COUNT=${{ matrix.shard-count }}" not in job:
...
grep -Fq "TokenCueTests\\.Alpha" "${FAKE_SWIFT_LOG}"
grep -Fq "TokenCueTests\\.Beta" "${FAKE_SWIFT_LOG}"
grep -Fq "TokenCueTests\\..*top\\ level\\ works" "${FAKE_SWIFT_LOG}"
grep -Fq "TokenCueTests\\..*top/level\\ slash\\ works" "${FAKE_SWIFT_LOG}"
[[ "$(wc -l < "${FAKE_SWIFT_LOG}")" -eq 5 ]]

reset_case strict
export FAKE_SWIFT_MODE=group_fail_once
set +e
TOKENCUE_TEST_GROUP_SIZE=4 \
  TOKENCUE_TEST_SUITE_TIMEOUT=10 \
  TOKENCUE_TEST_RETRY_NON_TIMEOUT_FAILURES=0 \
