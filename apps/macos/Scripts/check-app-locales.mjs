...
const resources = path.join(repoRoot, "Sources/TokenCue/Resources");
...
function readCatalog(locale) {
  ...
  // `plutil` is available on every supported macOS build host, but this
  // fallback keeps the static locale contract runnable from the Windows
  // monorepo checkout as well. Apple string catalogs in this repository use
  // the standard quoted `"key" = "value";` form.
  return parseStringsCatalog(fs.readFileSync(file, "utf8"));
}

function parseStringsCatalog(source) {
  const catalog = {};
  const entry = /"((?:\\.|[^"\\])*)"\s*=\s*"((?:\\.|[^"\\])*)"\s*;/gs;
  for (const match of source.matchAll(entry)) {
    catalog[decodeStringsLiteral(match[1])] = decodeStringsLiteral(match[2]);
  }
  return catalog;
}

function decodeStringsLiteral(value) {
  return value.replace(/\\(?:U([0-9a-fA-F]{4})|u([0-9a-fA-F]{4})|n|r|t|"|\\)/g, (escape, upper, lower) => {
    if (upper || lower) return String.fromCharCode(Number.parseInt(upper ?? lower, 16));
    return { "\\n": "\n", "\\r": "\r", "\\t": "\t", '\\"': '"', "\\\\": "\\" }[escape] ?? escape;
  });
}
