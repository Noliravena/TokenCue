...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/tokencue-repository-size.XXXXXX")
...
artifacts=(
  "TokenCue 2.app/Contents/MacOS/TokenCue"
  "TokenCue.dSYM/Contents/Info.plist"
  "TokenCue.xcarchive/Products/Applications/TokenCue.app/Contents/Info.plist"
  "TokenCue.xcresult/Data/data"
  "TokenCue.ipa"
  "TokenCue.zip"
  "TokenCue.delta"
  "TokenCue.dmg"
...
ln -s source.txt "$TEMP_DIR/TokenCue-latest.dmg"
git -C "$TEMP_DIR" add -f TokenCue-latest.dmg
rm "$TEMP_DIR/TokenCue.zip"
...
for artifact in "${artifacts[@]}" TokenCue-latest.dmg; do
