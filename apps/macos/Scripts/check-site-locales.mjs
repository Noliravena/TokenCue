...
const siteLocalesModule = path.join(repoRoot, "docs/site-locales.mjs");
if (!fs.existsSync(docsIndex) || !fs.existsSync(siteLocalesModule)) {
  console.log("skip site locale check: apps/macos/docs is not present in the TokenCue monorepo layout");
  process.exit(0);
}

const { localeCatalog, localeMessages } = await import("../docs/site-locales.mjs");
const indexHtml = fs.readFileSync(docsIndex, "utf8");
const providerSource = fs.readFileSync(
  path.join(repoRoot, "Sources/TokenCueCore/Providers/Providers.swift"),
  "utf8",
);
...
const publicCountFiles = [
  ["README.md", `alt="TokenCue — every AI coding limit in your menu bar. ${providerCount} providers."`],
  ["docs/providers.md", `TokenCue currently registers ${providerCount} provider IDs.`],
...
const appLanguageSource = fs.readFileSync(
  path.join(repoRoot, "Sources/TokenCue/PreferencesGeneralPane.swift"),
  "utf8",
);
...
for (const code of catalogCodes) {
  const hasHreflang =
    indexHtml.includes(`href="https://tokencue.app/?lang=${code}"`) ||
    indexHtml.includes(`href="https://github.com/Noliravena/TokenCue`) ||
    indexHtml.includes(`hreflang="${code}"`);
  assert(hasHreflang, `missing hreflang URL for ${code}`);
}
