...
struct HookDispatchTests {
    ...
    func `dispatch coalesces repeated refresh failures`() async throws {
        let output = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-hook-rate-limit-\(UUID().uuidString).json")
    ...
    func `dispatch contains one rule failure and continues`() async {
        let output = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-hook-failure-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: output) }
        let event = self.event()
        let config = HooksConfig(enabled: true, events: [
            HookRule(event: .quotaReached, executable: "/nonexistent/tokencue-hook"),
    ...
    func `disabled dispatch never invokes a rule`() async {
        let output = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-hook-disabled-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: output) }
