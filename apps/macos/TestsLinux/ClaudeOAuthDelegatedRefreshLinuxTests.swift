...
struct ClaudeOAuthDelegatedRefreshLinuxTests {
    ...
    func cliOAuthDoesNotDelegateRefreshEvenForUserAction() async {
        ...
        #expect(result.message.contains("TokenCue CLI does not launch Claude"))
    }
    ...
    func appOAuthBackgroundRespectsPlatformKeychainPromptPolicy() async {
        ...
        #expect(result.message.contains("Click Refresh in the TokenCue menu"))
        #expect(!result.message.contains("Open the TokenCue menu or"))
    }
