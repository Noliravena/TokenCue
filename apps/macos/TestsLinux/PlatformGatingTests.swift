...
struct PlatformGatingTests {
    ...
    func ampAutoSource_doesNotRequireWebSupport() {
        #expect(!TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .amp))
    }

    @Test
    func claudeAutoSource_allowsPlannerToFallBackToCLI() {
        #expect(!TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .claude))
        #expect(TokenCueCLI.sourceModeRequiresWebSupport(.web, provider: .claude))
    }
