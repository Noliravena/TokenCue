...
struct CLIGuardDecisionTests {
    ...
    func `ample headroom is ok and exits zero`() {
        let result = TokenCueCLI.evaluateGuard(
    ...
    func `unknown provider is rejected`() {
        let result = TokenCueCLI.guardProvider(rawOverride: "definitely-not-a-provider")
    ...
    func `missing provider is rejected`() {
        let result = TokenCueCLI.guardProvider(rawOverride: nil)
        guard case let .failure(error) = result else {
    ...
    func `timeout rejects values that could overflow duration`() {
        let result = TokenCueCLI.guardTimeout(raw: "1e100")
    ...
    func `fetch timeout is reported as unavailable`() async {
        let result = await TokenCueCLI.runGuardFetch(timeout: 0.01) {
    ...
    func `real window reports remaining headroom`() {
        let remaining = TokenCueCLI.guardRemainingHeadroom(for: self.window(usedPercent: 30, synthetic: false))
        #expect(remaining == 70)
    }

    @Test
    func `synthetic placeholder window is treated as unknown`() {
        let remaining = TokenCueCLI.guardRemainingHeadroom(for: self.window(usedPercent: 0, synthetic: true))
        #expect(remaining == nil)
    }

    @Test
    func `absent window is unknown`() {
        #expect(TokenCueCLI.guardRemainingHeadroom(for: nil) == nil)
    }

    @Test
    func `fully used real window has zero headroom`() {
        let remaining = TokenCueCLI.guardRemainingHeadroom(for: self.window(usedPercent: 100, synthetic: false))
        #expect(remaining == 0)
    }
}
