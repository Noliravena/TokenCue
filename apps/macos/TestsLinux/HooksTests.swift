...
struct HooksTests {
    ...
    func `environment variables include set fields and omit nil`() {
        ...
        #expect(env["TOKENCUE_EVENT"] == "quota_low")
        #expect(env["TOKENCUE_PROVIDER"] == "codex")
        #expect(env["TOKENCUE_WINDOW"] == "weekly")
        #expect(env["TOKENCUE_USAGE_PERCENT"] == "0.5")
        #expect(env["TOKENCUE_RESET_AT"] == "2023-11-14T22:13:20Z")
        #expect(env["TOKENCUE_TIMESTAMP"] != nil)
        #expect(env["TOKENCUE_ACCOUNT"] == nil)
        #expect(env["TOKENCUE_STATUS"] == nil)
    }
    ...
    func `runner executes command and passes event environment`() async throws {
        ...
        #expect(result.stdout.contains("TOKENCUE_EVENT=quota_reached"))
        #expect(result.stdout.contains("TOKENCUE_PROVIDER=codex"))
    }
    ...
    func `runner throws on missing executable`() async {
        let rule = HookRule(event: .quotaReached, executable: "/nonexistent/tokencue-hook")
