...
/// therefore take no `TokenCueConfig`, which makes the ordering defect structurally
...
struct CLIHooksWatchArgumentPrecedenceLinuxTests {
    ...
    func `interval below the floor is rejected without reading config`() {
        let result = TokenCueCLI.decodeHooksWatchInterval(from: Self.values(options: ["interval": ["5"]]))
    ...
    func `non numeric interval is rejected without reading config`() {
        let result = TokenCueCLI.decodeHooksWatchInterval(from: Self.values(options: ["interval": ["abc"]]))
        guard case let .failure(error) = result else {
    ...
    func `interval defaults when absent`() {
        let result = TokenCueCLI.decodeHooksWatchInterval(from: Self.values())
        guard case let .success(interval) = result else {
            Issue.record("expected the default interval")
            return
        }
        #expect(interval == TokenCueCLI.hooksWatchDefaultInterval)
    }

    @Test
    func `interval at the floor is accepted`() {
        let result = TokenCueCLI.decodeHooksWatchInterval(from: Self.values(options: ["interval": ["60"]]))
        guard case let .success(interval) = result else {
    ...
    func `unknown provider name is rejected without reading config`() {
        let result = TokenCueCLI.decodeHooksWatchProviderNames(
    ...
    func `absent provider option defers to config`() {
        let result = TokenCueCLI.decodeHooksWatchProviderNames(from: Self.values())
    ...
    func `explicit providers win over the enabled set`() {
        ...
        let result = TokenCueCLI.hooksWatchProviders(explicit: [.codex], config: config)
        guard case let .success(providers) = result else {
    ...
    func `empty enabled set reports no providers`() {
        ...
        let result = TokenCueCLI.hooksWatchProviders(explicit: nil, config: config)
        guard case let .failure(error) = result else {
