...
struct CLISnapshotTests {
    ...
    func `defaults to usage when no command provided`() {
        #expect(TokenCueCLI.effectiveArgv([]) == ["usage"])
        #expect(TokenCueCLI.effectiveArgv(["--format", "json"]).first == "usage")
        #expect(TokenCueCLI.effectiveArgv(["usage", "--format", "json"]).first == "usage")
    }
