    ...
    func `requests preserve credits headers and one second enrichment deadline`() async throws {
        ...
        let usage = try await runtime.fetchUsage(
            settings: [
                ...
                OpenRouterSettingsReader.httpRefererEnvironmentKey: "https://tokencue.example",
                OpenRouterSettingsReader.clientTitleEnvironmentKey: "TokenCue QA",
            ],
        ...
        #expect(recorded[0].value(forHTTPHeaderField: "HTTP-Referer") == "https://tokencue.example")
        #expect(recorded[0].value(forHTTPHeaderField: "X-Title") == "TokenCue QA")
