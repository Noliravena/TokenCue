...
///
/// Run with:
///   TOKENCUE_SCREENSHOT_DIR=docs/screenshots swift test --filter MenuLayoutScreenshotRenderTests
@MainActor
final class MenuLayoutScreenshotRenderTests: XCTestCase {
    ...
    func test_renderMultiAccountLayoutScreenshots() throws {
        guard let dir = ProcessInfo.processInfo.environment["TOKENCUE_SCREENSHOT_DIR"] else {
            throw XCTSkip("Set TOKENCUE_SCREENSHOT_DIR to render menu layout screenshots.")
        }
    ...
    func test_renderCachedCostRefreshScreenshots() throws {
        guard let dir = ProcessInfo.processInfo.environment["TOKENCUE_COST_SCREENSHOT_DIR"] else {
            throw XCTSkip("Set TOKENCUE_COST_SCREENSHOT_DIR to render cached cost screenshots.")
        }
        let directory = URL(fileURLWithPath: dir, isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
