...
struct CLIServeRouterTests {
    ...
    func `usage operation fingerprint separates dashboard account mode`() {
        let allAccounts = TokenCueCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true)
        let selectedAccount = TokenCueCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: false)

        #expect(allAccounts != selectedAccount)
        #expect(allAccounts == TokenCueCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true))
    }
    ...
    func `health response reports ok status and build version`() throws {
        let response = TokenCueCLI.serveHealthResponse(version: "1.2.3")
    ...
    func `health response omits version detail when unavailable`() throws {
        let response = TokenCueCLI.serveHealthResponse(version: nil)
        #expect(response.status == .ok)
        let object = try JSONSerialization.jsonObject(with: response.body) as? [String: Any]
        #expect(object?["status"] as? String == "ok")
    ...
    func `serve numeric options reject malformed values`() {
        #expect(TokenCueCLI.decodeServePort(from: ParsedValues(
            positional: [],
            options: ["port": ["abc"]],
            flags: [])) == nil)
        #expect(TokenCueCLI.decodeServePort(from: ParsedValues(
        ...
        #expect(TokenCueCLI.decodeServeRefreshInterval(from: ParsedValues(
            positional: [],
        ...
        #expect(TokenCueCLI.decodeServeRequestTimeout(from: ParsedValues(
            positional: [],
    ...
    func `serve help documents request timeout option`() {
        let serve = TokenCueCLI.serveHelp(version: "0.0.0")
        let root = TokenCueCLI.rootHelp(version: "0.0.0")

        #expect(serve.contains("--request-timeout <seconds>"))
        #expect(serve.contains("tokencue serve --port 8080 --refresh-interval 60 --request-timeout 30"))
    ...
    func `serve config snapshot reflects provider changes`() throws {
        ...
        var firstConfig = TokenCueConfig.makeDefault()
        firstConfig.setProviderConfig(ProviderConfig(id: .opencodego, enabled: false))
        try store.save(firstConfig)

        let firstSnapshot = try TokenCueCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let secondSnapshot = try TokenCueCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let operationKey = try TokenCueCLI.serveOperationKey(kind: "usage", provider: nil)
        #expect(try operationKey == (TokenCueCLI.serveOperationKey(kind: "usage", provider: nil)))
        #expect(
            TokenCueCLI.serveCacheKey(operationKey: operationKey, configToken: firstSnapshot.cacheToken) !=
                TokenCueCLI.serveCacheKey(operationKey: operationKey, configToken: secondSnapshot.cacheToken))
    }
    ...
    func `serve cache skips provider error payloads`() {
        ...
        #expect(TokenCueCLI.shouldCacheServeResponse(success))
        #expect(!TokenCueCLI.shouldCacheServeResponse(providerError))
        #expect(!TokenCueCLI.shouldCacheServeResponse(routeError))
    }

    @Test
    func `serve provider timeout stays below the request deadline`() throws {
        let thirtySecondTimeout = try #require(TokenCueCLI.serveProviderTimeout(requestTimeout: 30))
        let tenSecondTimeout = try #require(TokenCueCLI.serveProviderTimeout(requestTimeout: 10))
        ...
        #expect(TokenCueCLI.serveProviderTimeout(requestTimeout: 0) == nil)
        #expect(TokenCueCLI.serveProviderTimeout(requestTimeout: .infinity) == nil)
        // Finite deadlines stay strictly below the request timeout at every
        // value, including sub-second ones.
        let oneSecondTimeout = try #require(TokenCueCLI.serveProviderTimeout(requestTimeout: 1))
        let halfSecondTimeout = try #require(TokenCueCLI.serveProviderTimeout(requestTimeout: 0.5))
        ...
        let oversizedTimeout = try #require(TokenCueCLI.serveProviderTimeout(
    ...
    func `serve usage collection bounds a hung provider without blocking others`() async {
        ...
        let start = Date()
        let output = await TokenCueCLI.serveCollectUsageOutputs(
            providers: providers,
            providerTimeout: 0.1)
        { provider in
            if provider == .claude {
    ...
    func `serve cache uses stable Codex account identities`() {
        ...
        let firstKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: firstProjection)
        let reshapedKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: reshapedProjection)
        let replacementKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: replacement)
        let workspacePeerKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: workspacePeer)
        let storedBeforeKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: storedBeforeRefresh)
        let storedAfterKey = TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
        ...
        #expect(TokenCueCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
    ...
    func `serve cache coalesces concurrent cache misses`() async {
        ...
        let responses = await withTaskGroup(of: CLILocalHTTPResponse.self) { group -> [CLILocalHTTPResponse] in
            for _ in 0..<5 {
                group.addTask {
                    await TokenCueCLI.cachedServeResponse(
                        key: "usage:",
                        cache: cache,
                        refreshInterval: 60,
                        requestTimeout: 1)
    ...
    func `serve cache prunes expired config token entries`() async throws {
        ...
        _ = await TokenCueCLI.cachedServeResponse(
    ...
    func `serve cache does not cache timeouts and recovers on next success`() async {
        ...
        let timeout = await TokenCueCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 0.01)
        {
            ...
            do {
                ...
                return TokenCueCLI.serveTimeoutResponse()
            }
        }
        ...
        let success = await TokenCueCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
        ...
        let cached = await TokenCueCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
    ...
    func `serve cache serves last good payload when refresh fails`() async {
        ...
        let first = await TokenCueCLI.cachedServeResponse(
        ...
        let failed = await TokenCueCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
        ...
        let recovered = await TokenCueCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `cost refresh timeout serves the last good payload`() async throws {
        ...
        let timedOut = await TokenCueCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 0.01)
    ...
    func `cost refresh keeps fresh providers while replacing timed out rows`() async throws {
        ...
        let partial = await TokenCueCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 1)
    ...
    func `serve cache replaces only failed provider account rows`() async throws {
        ...
        let refreshed = await TokenCueCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve stale ttl is bounded and disabled without caching`() {
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: 0) == 0)
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: 1) == 300)
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: 60) == 600)
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: 1800) == 3600)
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: 86401) == 3600)
        #expect(TokenCueCLI.serveStaleTTL(refreshInterval: .infinity) == 3600)
    }
    ...
    func `serve helper idle window outlives the refresh cadence`() {
        #expect(TokenCueCLI.serveCLISessionIdleWindow(refreshInterval: 0) == 180)
        #expect(TokenCueCLI.serveCLISessionIdleWindow(refreshInterval: 60) == 180)
        #expect(TokenCueCLI.serveCLISessionIdleWindow(refreshInterval: 300) == 360)
    }
    ...
    func `serve request timeout zero disables the deadline`() async {
        ...
        let response = await TokenCueCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0,
            requestTimeout: 0)
