...
/// Raw-socket coverage for `tokencue serve`: boots the real `CLILocalHTTPServer` on an
...
struct CLIServeRawHTTPTests {
    ...
    func `web UI returns HTML with no-store`() async throws {
        try await Self.withServeRuntime(token: nil, body: { port in
            ...
            #expect(response.headerValue("Cache-Control") == "no-store")
            #expect(response.body.contains("TokenCue Dashboard"))
    ...
    func `snapshot response preserves usage cache metadata`() async throws {
        ...
        try store.save(TokenCueConfig(providers: UsageProvider.allCases.map {
        ...
        let response = await TokenCueCLI.handleServeRequest(request, runtime: runtime)
        ...
        body: (UInt16) async throws -> Void) async throws
    {
        ...
        try await Self.withServer(
            handler: { request in
                await TokenCueCLI.handleServeRequest(request, runtime: runtime)
            },
            body: body)
    }
