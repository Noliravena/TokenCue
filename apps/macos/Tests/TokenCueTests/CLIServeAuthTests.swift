...
struct CLIServeAuthTests {
    ...
    func `serve help documents dashboard token transport honestly`() {
        ...
        #expect(serve.contains("TOKENCUE_DASHBOARD_TOKEN"))
    ...
    func `serve host option parses and normalizes`() {
        #expect(TokenCueCLI.decodeServeHost(from: ParsedValues(
            positional: [],
            options: [:],
            flags: [])) == "127.0.0.1")
        #expect(TokenCueCLI.decodeServeHost(from: ParsedValues(
    ...
    func `serve flags parse through the real commander signature`() throws {
        ...
        #expect(TokenCueCLI.decodeServeHost(from: values) == "0.0.0.0")
        #expect(TokenCueCLI.decodeServeAllowPlainHTTP(from: values))
        #expect(TokenCueCLI.resolveDashboardToken(from: values, environment: [:]) == .token("secret"))
        #expect(TokenCueCLI.decodeServeHost(from: defaults) == "127.0.0.1")
        #expect(!TokenCueCLI.decodeServeAllowPlainHTTP(from: defaults))
        #expect(TokenCueCLI.resolveDashboardToken(from: defaults, environment: [:]) == .absent)
    }
    ...
    func `dashboard token resolution prefers the environment and rejects blanks`() {
        ...
        let envOverride = Dictionary(uniqueKeysWithValues: [
            (TokenCueCLI.dashboardTokenEnvironmentVariable, "ENV_VALUE"),
        ])

        #expect(TokenCueCLI.resolveDashboardToken(
            from: emptyValues,
            environment: [:]) == .absent)
        #expect(TokenCueCLI.resolveDashboardToken(
            from: flagValues,
        ...
        #expect(TokenCueCLI.resolveDashboardToken(
            from: emptyValues,
            environment: ["TOKENCUE_DASHBOARD_TOKEN": "  "])
            == .empty(source: "TOKENCUE_DASHBOARD_TOKEN"))
        #expect(TokenCueCLI.resolveDashboardToken(
    ...
    func `serve startup validation enforces the token and plain-http matrix`() {
        ...
        #expect(TokenCueCLI.validateServeStartup(
            host: "127.0.0.1",
    ...
    func `adding no-store preserves an existing cache-control header`() {
        ...
        let annotated = TokenCueCLI.addingNoStore(plain)
        let untouched = TokenCueCLI.addingNoStore(declared)

        #expect(annotated.extraHeaders.contains { $0 == ("Cache-Control", "no-store") })
        #expect(untouched.extraHeaders.count == 1)
        #expect(TokenCueCLI.addingNoStore(annotated).extraHeaders.count == 1)
    }

    @Test
    func `unauthorized response advertises bearer challenge and no-store`() {
        let response = TokenCueCLI.serveUnauthorizedResponse()
