...
struct CLICostTests {
    ...
    func `cost json shortcut does not enable json logs`() throws {
        let signature = TokenCueCLI._costSignatureForTesting()
    ...
    func `provider native only excludes pi and OMP session mirrors`() throws {
        let parser = CommandParser(signature: TokenCueCLI._costSignatureForTesting())

        let defaultValues = try parser.parse(arguments: [])
        #expect(TokenCueCLI.decodeCostIncludePiSessions(from: defaultValues))

        let nativeOnlyValues = try parser.parse(arguments: ["--provider-native-only"])
        #expect(!TokenCueCLI.decodeCostIncludePiSessions(from: nativeOnlyValues))
    }
    ...
    func `renders cost text snapshot`() {
        ...
        let output = TokenCueCLI.renderCostText(provider: .claude, snapshot: snap, useColor: false)
    ...
    func `renders codex project grouped cost text`() {
        ...
        let output = TokenCueCLI.renderCostText(
            provider: .codex,
            snapshot: snap,
            groupBy: .project,
            useColor: false)
    ...
    func `cost JSON exposes history coverage as a boolean`() throws {
        for coverage in [false, true] {
            let snapshot = CostUsageTokenSnapshot(
                ...
                daily: [],
                updatedAt: Date(timeIntervalSince1970: 1_700_000_000))
            let payload = TokenCueCLI.makeCostPayload(provider: .codex, snapshot: snapshot, error: nil)
    ...
    func `cursor cookie source off produces a failed JSON payload`() throws {
        ...
        let error = try #require(TokenCueCLI.cursorCostAvailabilityError(.cursor, settings: settings))
        let payload = TokenCueCLI.makeCostPayload(provider: .cursor, snapshot: nil, error: error)
        let json = try #require(TokenCueCLI.encodeJSON([payload], pretty: false))

        #expect(TokenCueCLI.mapError(error) == .failure)
        #expect(json.contains("\"provider\":\"cursor\""))
        #expect(json.contains("\"code\":1"))
        #expect(json.contains("cookie source is set to Off"))
        #expect(TokenCueCLI.cursorCostAvailabilityError(.cursor, settings: nil) == nil)
        #expect(TokenCueCLI.cursorCostAvailabilityError(.codex, settings: settings) == nil)
    }
