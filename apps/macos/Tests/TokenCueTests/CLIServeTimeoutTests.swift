...
struct CLIServeTimeoutTests {
    ...
    func `dashboard awaits pricing refresh while serve refreshes in background`() {
        #expect(!TokenCueCLI.dashboardCostRefreshesPricingInBackground)
        #expect(TokenCueCLI.serveCostRefreshesPricingInBackground)
    }

    @Test
    func `serve deadlines clamp once from request entry`() throws {
        #expect(TokenCueCLI.clampedServeRequestTimeout(.greatestFiniteMagnitude) == 86400)
        #expect(TokenCueCLI.clampedServeRequestTimeout(1e308) == 86400)
        #expect(TokenCueCLI.clampedServeRequestTimeout(-5) == 0)

        let startedAt = ContinuousClock().now
        let deadline = try #require(TokenCueCLI.serveRequestDeadline(
            startedAt: startedAt,
            requestTimeout: .greatestFiniteMagnitude))
        #expect(startedAt.duration(to: deadline) == .seconds(86400))
        #expect(TokenCueCLI.serveRequestDeadline(startedAt: startedAt, requestTimeout: 0) == nil)

        let requestDeadline = startedAt.advanced(by: .seconds(40))
        #expect(TokenCueCLI.serveCostProviderDeadline(
            startedAt: startedAt,
            providerTimeout: 30,
    ...
    func `late response warms the cache for the next same config request`() async {
        ...
        let next = await TokenCueCLI.cachedServeResponse(
            key: "dashboard:v1:snapshot:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 30)
    ...
    func `provider timeout preserves healthy rows and cannot stack provider work`() async {
        ...
        let first = Task {
            await TokenCueCLI.serveCollectUsageOutputs(
    ...
    func `cost route variants cannot stack the same provider scan`() async {
        let clock = ServeManualDeadlineClock()
        let late = TokenCueCLI.makeCostPayload(provider: .claude, snapshot: nil, error: nil)
        ...
        let first = Task {
            await TokenCueCLI.serveCollectCostPayloads(
            ...
            { provider in
                ...
                return TokenCueCLI.makeCostPayload(provider: provider, snapshot: nil, error: nil)
            }
        }
        await blocked.waitForStarts(1)
        await clock.waitForPendingSleeps(1)
