...
struct TTYCommandRunnerEnvTests {
    ...
    func `codex status probe uses non persistent thread storage`() {
        let stateHome = URL(fileURLWithPath: "/tmp/tokencue status \"state\"", isDirectory: true)
        ...
        #expect(args.contains("experimental_thread_store={type=\"in_memory\",id=\"tokencue-status\"}"))
        #expect(args.contains("sqlite_home=\"/tmp/tokencue status \\\"state\\\"\""))
    }
    ...
    func `sets working directory when provided`() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("tokencue-tty-\(UUID().uuidString)", isDirectory: true)
