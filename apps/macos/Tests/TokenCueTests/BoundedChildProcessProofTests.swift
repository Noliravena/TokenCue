...
struct BoundedChildProcessProofTests {
    ...
    func `synthetic PTY child overflow propagates and cleans up the process`() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("TokenCueBoundedProcessProof-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["TOKENCUE_PROOF_PID_FILE"] = pidURL.path
    ...
    func `synthetic Grok RPC child returns a normal framed response`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("TokenCueBoundedRPCProof-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func `synthetic Grok RPC child overflow terminates the process`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("TokenCueBoundedRPCOverflowProof-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        let client = try GrokRPCClient(
            ...
            environment: [
                "PATH": "/usr/bin:/bin",
                "GROK_CLI_PATH": scriptURL.path,
                "TOKENCUE_PROOF_PID_FILE": pidURL.path,
            ],
