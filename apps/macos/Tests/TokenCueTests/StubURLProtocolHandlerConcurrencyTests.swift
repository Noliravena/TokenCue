...
///
/// Opt-in: it hammers a static thousands of times, so it is gated behind `TOKENCUE_TSAN_STRESS` and
/// run in isolation via `TOKENCUE_TSAN_STRESS=1 swift test --sanitize=thread --filter
/// StubURLProtocolHandlerConcurrencyTests`, never in the normal parallel suite.
@Suite(.serialized)
struct StubURLProtocolHandlerConcurrencyTests {
    @Test(.enabled(if: ProcessInfo.processInfo.environment["TOKENCUE_TSAN_STRESS"] == "1"))
