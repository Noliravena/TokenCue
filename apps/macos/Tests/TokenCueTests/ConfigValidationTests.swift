...
struct ConfigValidationTests {
    ...
    func `reports unsafe hook rule fields`() {
        ...
        let codes = Set(TokenCueConfigValidator.validate(config).map(\.code))
    ...
    func `reports hook workload limits`() {
        ...
        let config = TokenCueConfig(providers: [], hooks: HooksConfig(enabled: true, events: rules))
        let codes = Set(TokenCueConfigValidator.validate(config).map(\.code))
    ...
    func `fresh config defaults Alibaba Token Plan to International`() throws {
        ...
        let issues = TokenCueConfigValidator.validate(config)
    ...
    func `sub2api accepts HTTPS and loopback HTTP base URLs`() {
        for host in ["https://sub2api.example.com", "http://127.0.0.1:8080"] {
            ...
            config.setProviderConfig(ProviderConfig(
                ...
                apiKey: "fixture",
                enterpriseHost: host))
            let invalidHostIssue = TokenCueConfigValidator.validate(config).first { issue in
    ...
    func `unsupported enterprise host warning lists every supported provider`() throws {
        ...
        let issue = try #require(TokenCueConfigValidator.validate(config).first(where: {
    ...
    func `config store default url honors environment override`() {
        let url = TokenCueConfigStore.defaultURL(environment: [
            TokenCueConfigStore.pathEnvironmentKey: "~/tmp/tokencue-test-config.json",
        ])

        #expect(url.path.hasSuffix("/tmp/tokencue-test-config.json"))
    }
    ...
    func `config store default url honors xdg config home`() throws {
        ...
        let url = TokenCueConfigStore.defaultURL(
            home: home,
            environment: [
                TokenCueConfigStore.xdgConfigHomeEnvironmentKey: xdgHome.path,
            ],
    ...
    func `config store default url ignores relative xdg config home`() throws {
        ...
        let url = TokenCueConfigStore.defaultURL(
            home: home,
            environment: [
                TokenCueConfigStore.xdgConfigHomeEnvironmentKey: "relative-config",
            ],
            fileManager: fileManager)
    ...
    func `config store default url creates in xdg default for new installs`() throws {
        ...
        let url = TokenCueConfigStore.defaultURL(home: home, environment: [:], fileManager: fileManager)
    ...
    private static func makeTemporaryHome() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("TokenCueConfigStoreTests-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func configURL(in directory: URL) -> URL {
        directory
            .appendingPathComponent("tokencue", isDirectory: true)
