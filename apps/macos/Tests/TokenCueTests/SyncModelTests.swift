...
struct SyncModelTests {
    ...
    func `malicious intent hooks never alter local hooks`() throws {
        ...
        var config = TokenCueConfig(providers: [ProviderConfig(id: .codex, enabled: true)], hooks: hooks)
    ...
    func `newer schema version requires an app update`() {
        #expect(TokenCueSyncSchema.canApply(1))
        #expect(!TokenCueSyncSchema.canApply(2))
    }
