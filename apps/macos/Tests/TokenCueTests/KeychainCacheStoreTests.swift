...
struct KeychainCacheStoreTests {
    ...
    func `cache ACL trusts bundled app and CLI helper`() {
        let root = URL(fileURLWithPath: "/Applications/TokenCue.app")
        let executable = root.appendingPathComponent("Contents/MacOS/TokenCue")
        let helper = root.appendingPathComponent("Contents/Helpers/TokenCueCLI")
        let existing = Set([
            root.path,
            executable.path,
            helper.path,
    ...
    func `cache ACL refuses bare dev binaries without an app bundle`() {
        ...
        let executable = URL(fileURLWithPath: "/Users/dev/project/.build/debug/TokenCueCLI")
