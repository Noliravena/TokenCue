...
struct DashboardSnapshotBuilderTests {
    ...
    func `dashboard cost collection does not fetch Cursor when cookie source is off`() async {
        ...
        let payload = await TokenCueCLI.collectConfiguredCostPayloads(
            providers: [.cursor],
            config: config,
    ...
    func `builds stable display-oriented dashboard snapshot`() throws {
        ...
        #expect(host["tokenCueVersion"] as? String == "9.8.7")
    ...
    func `dashboard identity mode none emits null identity`() throws {
        ...
        let snapshot = DashboardSnapshotBuilder.makeSnapshot(
            usagePayloads: [payload],
            costPayloads: [],
            config: TokenCueConfig(providers: [ProviderConfig(id: .claude, enabled: true)]),
            identityMode: .none,
    ...
    func `dashboard labels amp subscription pools as provider specific windows`() throws {
        ...
        let snapshot = DashboardSnapshotBuilder.makeSnapshot(
            usagePayloads: [payload],
            costPayloads: [],
            config: TokenCueConfig(providers: [ProviderConfig(id: .amp, enabled: true)]),
            identityMode: .redacted,
            generatedAt: now,
            refreshInterval: 60,
            tokenCueVersion: nil)
    ...
    func `dashboard provider errors are projected without raw usage internals`() throws {
        ...
        let snapshot = DashboardSnapshotBuilder.makeSnapshot(
            usagePayloads: [payload],
            costPayloads: [],
            config: TokenCueConfig(providers: [ProviderConfig(id: .codex, enabled: true)]),
            identityMode: .redacted,
            generatedAt: Date(timeIntervalSince1970: 0),
            refreshInterval: 60,
            tokenCueVersion: nil)
...
private actor DashboardCostConfigRecorder {
    ...
    func record(providers: [UsageProvider], config: TokenCueConfig) {
