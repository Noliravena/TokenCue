    ...
    func `request scopes auth and account with bounded timeout`() async throws {
        ...
        let snapshot = try await CodexAuthenticatedHTTPTransport.$overrideForTesting.withValue(transport) {
            try await CodexOAuthUsageFetcher.fetchRateLimitResetCredits(
                ...
                env: ["CODEX_HOME": "/tmp/tokencue-reset-credit-request-test"])
        }
    ...
    func `rejects negative available count`() async throws {
        ...
        do {
            _ = try await CodexOAuthUsageFetcher.fetchRateLimitResetCredits(
                accessToken: "test-token",
                accountId: nil,
                env: ["CODEX_HOME": "/tmp/tokencue-negative-reset-credit-test"],
                session: transport)
    ...
    func `reset credit GET preserves transport cancellation`() async throws {
        ...
        await #expect(throws: CancellationError.self) {
            _ = try await CodexOAuthUsageFetcher.fetchRateLimitResetCredits(
                accessToken: "test-token",
                accountId: "account-123",
                env: ["CODEX_HOME": "/tmp/tokencue-reset-credit-cancellation-test"],
                session: transport)
        }
    }
