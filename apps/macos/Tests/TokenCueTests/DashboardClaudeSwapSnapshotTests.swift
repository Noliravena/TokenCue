...
struct DashboardClaudeSwapSnapshotTests {
    ...
    func `dashboard identity flag decodes redacted full and rejects others`() {
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: nil)) == .redacted)
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: "redacted")) == .redacted)
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: "full")) == .full)
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: "FULL")) == .full)
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: "none")) == nil)
        #expect(TokenCueCLI.decodeDashboardIdentityMode(from: self.parsedValues(identity: "bogus")) == nil)
    }
    ...
    func `dashboard output flag decodes stdout default file paths and rejects empty`() {
        #expect(TokenCueCLI.decodeDashboardOutputDestination(from: self.parsedValues(output: nil)) == .stdout)
        #expect(TokenCueCLI.decodeDashboardOutputDestination(from: self.parsedValues(output: "/tmp/snapshot.json"))
            == .file("/tmp/snapshot.json"))
        #expect(TokenCueCLI.decodeDashboardOutputDestination(from: self.parsedValues(output: "relative.json"))
            == .file("relative.json"))
        #expect(TokenCueCLI.decodeDashboardOutputDestination(from: self.parsedValues(output: "")) == nil)
    }
    ...
    func `producer collects swap accounts with config while its default omits them`() async throws {
        ...
        let result = try await producer.collect(config: config, refreshInterval: 0, tokenCueVersion: nil)
    ...
    func `dashboard swap eligibility requires enabled Claude and integration opt in`() {
        ...
        #expect(!TokenCueCLI.dashboardClaudeSwapIsEligible(config: TokenCueConfig(providers: [disabled])))
        #expect(!TokenCueCLI.dashboardClaudeSwapIsEligible(config: TokenCueConfig(providers: [unset])))
        #expect(!TokenCueCLI.dashboardClaudeSwapIsEligible(config: TokenCueConfig(providers: [explicitFalse])))
        #expect(TokenCueCLI.dashboardClaudeSwapIsEligible(config: TokenCueConfig(providers: [enabled])))
    }
        ...
        claudeSwap: DashboardClaudeSwapInput?) throws -> [[String: Any]]
    {
        let snapshot = DashboardSnapshotBuilder.makeSnapshot(
            ...
            costPayloads: [],
            config: TokenCueConfig(providers: [
...
private actor DashboardClaudeSwapConfigRecorder {
    ...
    func record(_ config: TokenCueConfig) {
