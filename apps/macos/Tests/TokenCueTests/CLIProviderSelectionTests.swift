...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = TokenCueCLI.usageHelp(version: "0.0.0")
        ...
        #expect(usage.contains("tokencue usage --provider gemini"))
        #expect(usage.contains("tokencue usage --format json --provider all --pretty"))
        #expect(root.contains("tokencue --provider gemini"))
    }
    ...
    func `provider selection respects override`() {
        let selection = TokenCueCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
    ...
    func `provider selection does not expand three enabled providers to all providers`() {
        ...
        let selection = TokenCueCLI.providerSelection(rawOverride: nil, enabled: enabled)
    ...
    func `provider selection accepts kiro alias`() {
        let selection = TokenCueCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])
