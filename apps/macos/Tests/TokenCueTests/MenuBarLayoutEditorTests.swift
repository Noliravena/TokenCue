...
struct MenuBarLayoutEditorTests {
    ...
    func `palette drag transfer representation round trips`() async throws {
        ...
        #expect(MenuBarLayoutDragItem.exportedContentTypes() == [.tokenCueMenuLayoutItem])
        #expect(MenuBarLayoutDragItem.importedContentTypes() == [.tokenCueMenuLayoutItem])

        let data = try await payload.exported(as: .tokenCueMenuLayoutItem)
        let decoded = try await MenuBarLayoutDragItem(
            importing: data,
            contentType: .tokenCueMenuLayoutItem)
