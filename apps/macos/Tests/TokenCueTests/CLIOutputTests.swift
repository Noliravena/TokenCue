...
struct CLIOutputTests {
    ...
    func `cli error payload is JSON array`() throws {
        let payload = TokenCueCLI.makeCLIErrorPayload(
            message: "Nope",
            code: .failure,
            kind: .args,
            pretty: false)
    ...
    func `exit omits generic error when command already emitted payload`() {
        #expect(!TokenCueCLI.shouldPrintExitError(code: .success, message: nil))
        #expect(!TokenCueCLI.shouldPrintExitError(code: .failure, message: nil))
        #expect(TokenCueCLI.shouldPrintExitError(code: .failure, message: "Nope"))
    }
