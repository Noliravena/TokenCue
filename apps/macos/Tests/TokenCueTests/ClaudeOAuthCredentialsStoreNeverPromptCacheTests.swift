...
struct ClaudeOAuthCredentialsStoreNeverPromptCacheTests {
    ...
    func `bundled CLI resolves the owning app prompt policy domain`() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let appURL = tempDirectory.appendingPathComponent("TokenCue.app", isDirectory: true)
        ...
        try Data().write(to: macOSURL.appendingPathComponent("TokenCue"))
        ...
        let debugWidgetDomain = ClaudeOAuthKeychainPromptPreference.resolveApplicationDefaultsDomain(
            bundleIdentifier: "com.tokencue.desktop.debug.widget",
            bundleURL: nil,
            executableURL: nil,
            invocationURL: nil)
        ...
        let standaloneDomain = ClaudeOAuthKeychainPromptPreference.resolveApplicationDefaultsDomain(
            bundleIdentifier: nil,
            bundleURL: nil,
            executableURL: URL(fileURLWithPath: "/usr/local/bin/tokencue"),
            invocationURL: nil)
