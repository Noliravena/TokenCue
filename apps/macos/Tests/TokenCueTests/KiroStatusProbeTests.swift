...
struct KiroStatusProbeTests {
    ...
    func `accepted pipe output cannot overrun the usage deadline`() async throws {
        let pipePIDFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-deadline-\(UUID().uuidString).pid")
        let ptyMarker = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-deadline-\(UUID().uuidString).pty")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
...
extension KiroStatusProbeTests {
    ...
    func `pipe cleanup finishes before PTY fallback starts`() async throws {
        let childPIDFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-pipe-child-\(UUID().uuidString).pid")
        let ptyMarker = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-pty-fallback-\(UUID().uuidString).started")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
    ...
    func `shutdown registry terminates an active pipe probe`() async throws {
        let pipePIDFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-shutdown-\(UUID().uuidString).pid")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
    ...
    func `fetch cancellation during context probe is preserved`() async throws {
        let contextStarted = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-context-\(UUID().uuidString).started")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
    ...
    func `cancellation during pipe cleanup wins over an expired context deadline`() async throws {
        let cleanupStarted = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-cleanup-\(UUID().uuidString).started")
    ...
    func `fetch cancellation while waiting for account probe is preserved`() async throws {
        let accountStarted = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-account-\(UUID().uuidString).started")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
    ...
    func `fetch returns promptly when usage helper spawns a detached child`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-pipe-\(UUID().uuidString)", isDirectory: true)
        ...
        child = subprocess.Popen(
            [
                ...
                os.environ["TOKENCUE_TEST_CHILD_PID_FILE"],
                str(ready_write),
            ],
        ...
        os.close(ready_read)
        '
          test -s "$TOKENCUE_TEST_CHILD_PID_FILE"
          printf 'Estimated Usage | resets on 2026-06-01 | KIRO FREE\\n'
          printf 'Credits (12.50 of 50 covered in plan)\\n'
          printf '████████████████████ 25%%\\n'
          exit 0
        ...
        let previousPIDFile = ProcessInfo.processInfo.environment["TOKENCUE_TEST_CHILD_PID_FILE"]
        setenv("TOKENCUE_TEST_CHILD_PID_FILE", childPIDFile.path, 1)
        defer {
            if let previousPIDFile {
                setenv("TOKENCUE_TEST_CHILD_PID_FILE", previousPIDFile, 1)
            } else {
                unsetenv("TOKENCUE_TEST_CHILD_PID_FILE")
            }
        }
    ...
    func `tty runner kills a pipe holder that escapes the process group`() async throws {
        let childPIDFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-escaped-\(UUID().uuidString).pid")
        let cliURL = try self.makeCLI(
            """
            #!/usr/bin/python3
            import subprocess
    ...
    func `tty runner cleans a same group helper after normal exit`() async throws {
        let childPIDFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-normal-exit-\(UUID().uuidString).pid")
        let cliURL = try self.makeCLI(
            """
            #!/usr/bin/python3
            import os
    ...
    func `tty runner cancellation terminates the process`() async throws {
        let pidFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-cancel-\(UUID().uuidString).pid")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            printf '%s\\n' "$$" > "$1"
...
extension KiroStatusProbeTests {
    ...
    private func makeCLI(_ script: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-cli-\(UUID().uuidString)", isDirectory: true)
        let cliURL = root.appendingPathComponent("kiro-cli")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        try script.write(to: cliURL, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: cliURL.path)
...
extension KiroStatusProbeTests {
    ...
    func `fetch cancellation while joining account after usage failure is preserved`() async throws {
        let accountStarted = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-kiro-failed-usage-account-\(UUID().uuidString).started")
        let cliURL = try self.makeCLI(
            """
            #!/bin/sh
            if [ "$1" = "whoami" ]; then
