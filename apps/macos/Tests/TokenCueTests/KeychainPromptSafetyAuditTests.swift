...
struct KeychainPromptSafetyAuditTests {
    ...
    func `default test runner explicitly suppresses real keychain access`() throws {
        ...
        #expect(script.contains("TOKENCUE_ALLOW_TEST_KEYCHAIN_ACCESS"))
        #expect(script.contains("export TOKENCUE_SUPPRESS_TEST_KEYCHAIN_ACCESS=1"))
    }

    @Test
    func `live TTY integration tests are opt in`() throws {
        let ttyTests = try Self.readRepoFile("Tests/TokenCueTests/TTYIntegrationTests.swift")
    ...
    func `claude availability tests with keychain enabled use test doubles`() throws {
        let file = Self.repoRoot().appendingPathComponent(
            "Tests/TokenCueTests/ClaudeOAuthFetchStrategyAvailabilityTests.swift")
    ...
    func `tests do not call Security item APIs except no UI query coverage`() throws {
        ...
        let offenders = try Self.swiftTestFiles().filter { file in
            ...
            return securityItemCalls.contains(where: text.contains)
                && !file.path.hasSuffix("Tests/TokenCueTests/KeychainNoUIQueryTests.swift")
                && !file.path.hasSuffix("Tests/TokenCueTests/KeychainPromptSafetyAuditTests.swift")
        }
    ...
    func `production source routes Security item APIs through the test safety gateway`() throws {
        ...
        let offenders = try Self.swiftFiles(
            ...
            .filter { file in
                guard !file.path.hasSuffix("Sources/TokenCueCore/KeychainSecurity.swift") else { return false }
                let text = try Self.readFile(file)
                return securityItemCalls.contains(where: text.contains)
            }
    ...
    func `production source resolves Security symbols via dlsym only in audited files`() throws {
        ...
        let allowedFiles = [
            "Sources/TokenCueCore/KeychainCacheStore.swift",
            ...
            "Sources/TokenCueCore/KeychainAccessPreflight.swift",
            "Sources/TokenCueCore/KeychainNoUIQuery.swift",
            "Sources/TokenCueCore/KeychainSecurity.swift",
        ]
        let offenders = try Self.swiftFiles(
            under: Self.repoRoot().appendingPathComponent("Sources", isDirectory: true))
            .filter { file in
    ...
    private static func swiftTestFiles(excludingSelf: Bool = false) throws -> [URL] {
        let testsRoot = self.repoRoot().appendingPathComponent("Tests/TokenCueTests", isDirectory: true)
        return try self.swiftFiles(under: testsRoot).filter { file in
            !(excludingSelf && file.path.hasSuffix("Tests/TokenCueTests/KeychainPromptSafetyAuditTests.swift"))
        }
    }
