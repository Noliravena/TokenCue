...
@Suite(.serialized)
struct TokenCueConfigMigratorTests {
    @Test
    func `legacy Moonshot key is bound to its selected region`() throws {
        let suite = "TokenCueConfigMigratorTests-moonshot-\(UUID().uuidString)"
        ...
        let migrated = TokenCueConfigMigrator.loadOrMigrate(
    ...
    func `legacy secret migration completion flag skips repeated scans`() throws {
        let suite = "TokenCueConfigMigratorTests-skip-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
        ...
        _ = TokenCueConfigMigrator.loadOrMigrate(configStore: configStore, userDefaults: defaults, stores: stores)
    ...
    func `legacy migration completion waits for successful cleanup`() throws {
        let suite = "TokenCueConfigMigratorTests-cleanup-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
    ...
    func `legacy stores are kept when migrated config save fails`() throws {
        let suite = "TokenCueConfigMigratorTests-save-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }

        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-tests", isDirectory: true)
        ...
        let accountStore = CountingTokenAccountStore()
        let stores = Self.legacyStores(secrets: secrets, accountStore: accountStore)
        let configStore = TokenCueConfigStore(
    ...
    private static let legacyMigrationCompletedKey = "tokencue.legacySecretsMigrationCompleted"

    private static func legacyStores(
        secrets: CountingLegacySecretStore,
        accountStore: CountingTokenAccountStore) -> TokenCueConfigMigrator.LegacyStores
    {
        TokenCueConfigMigrator.LegacyStores(
...
private final class CountingTokenAccountStore: ProviderTokenAccountStoring, @unchecked Sendable {
    ...
    func ensureFileExists() throws -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("tokencue-empty-accounts.json")
    }
}
