...
final class CLIEntryTests: XCTestCase {
    func test_effectiveArgvDefaultsToUsage() {
        XCTAssertEqual(TokenCueCLI.effectiveArgv([]), ["usage"])
        XCTAssertEqual(TokenCueCLI.effectiveArgv(["--json"]), ["usage", "--json"])
        XCTAssertEqual(TokenCueCLI.effectiveArgv(["usage", "--json"]), ["usage", "--json"])
    }

    func test_rootHelpAdvertisesDashboardSnapshotCommand() {
        let help = TokenCueCLI.rootHelp(version: "0.0.0")

        XCTAssertTrue(help.contains("tokencue dashboard [--pretty] [--timeout <seconds>] [--output <path>]"))
    }

    func test_dashboardCommandIsRegisteredAndParsesOptions() throws {
        let program = Program(descriptors: TokenCueCLI.commandDescriptors())
    ...
    func test_dashboardTimeoutIsBoundedAndCanBeDisabled() {
        XCTAssertEqual(
            TokenCueCLI.decodeDashboardTimeout(from: ParsedValues(positional: [], options: [:], flags: [])),
            30)
        XCTAssertEqual(
            TokenCueCLI.decodeDashboardTimeout(
        ...
        for value in ["-1", "nan", "inf", "86401"] {
            XCTAssertNil(TokenCueCLI.decodeDashboardTimeout(
    ...
    func test_dashboardCommandPrintsOneSnapshotAndExits() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-dashboard-command-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        var config = TokenCueConfig.makeDefault()
        ...
        try TokenCueConfigStore(fileURL: configURL).save(config)

        let result = try Self.runCLI(
            arguments: ["dashboard"],
            environment: [TokenCueConfigStore.pathEnvironmentKey: configURL.path])
    ...
    func test_dashboardOutputWritesSnapshotFileAndKeepsStdoutSilent() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-dashboard-output-\(UUID().uuidString)", isDirectory: true)
        ...
        let leftovers = try FileManager.default.contentsOfDirectory(atPath: root.path)
            .filter { $0.contains("tokencue-dashboard-") }
    ...
    func test_dashboardAtomicWriteFailsWhenDirectoryIsMissing() {
        let missing = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-missing-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("snapshot.json")

        XCTAssertThrowsError(
            try TokenCueCLI.writeDashboardSnapshotAtomically(Data("{}".utf8), toPath: missing.path))
        { error in
    ...
    func test_dashboardAtomicWriteReplacesExistingFile() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-atomic-write-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        try TokenCueCLI.writeDashboardSnapshotAtomically(Data("new".utf8), toPath: target.path)
    ...
    func test_decodesFormatFromOptionsAndFlags() {
        ...
        XCTAssertEqual(TokenCueCLI._decodeFormatForTesting(from: jsonOption), .json)

        let jsonFlag = ParsedValues(positional: [], options: [:], flags: ["json"])
        XCTAssertEqual(TokenCueCLI._decodeFormatForTesting(from: jsonFlag), .json)

        let textDefault = ParsedValues(positional: [], options: [:], flags: [])
        XCTAssertEqual(TokenCueCLI._decodeFormatForTesting(from: textDefault), .text)
    }

    func test_providerSelectionPrefersOverride() {
        let selection = TokenCueCLI.providerSelection(rawOverride: "codex", enabled: [.claude, .gemini])
        XCTAssertEqual(selection.asList, [.codex])
    }

    func test_normalizeVersionExtractsNumeric() {
        XCTAssertEqual(TokenCueCLI.normalizeVersion(raw: "codex 1.2.3 (build 4)"), "1.2.3")
        XCTAssertEqual(TokenCueCLI.normalizeVersion(raw: "  v2.0  "), "2.0")
    }

    func test_makeHeaderIncludesVersionWhenAvailable() {
        let header = TokenCueCLI.makeHeader(provider: .codex, version: "1.2.3", source: "cli")
    ...
    func test_cliVersionFallsBackToContainingAppBundle() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-cli-version-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let appURL = root.appendingPathComponent("TokenCue.app", isDirectory: true)
        ...
        let helperURL = helpersURL.appendingPathComponent("TokenCueCLI")
        try Data().write(to: helperURL)

        XCTAssertEqual(TokenCueCLI.containingAppVersion(for: helperURL), "9.8.7")
    }

    func test_cliVersionFollowsSymlinkedHelper() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-cli-version-symlink-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let symlinkURL = binURL.appendingPathComponent("tokencue")
        try FileManager.default.createSymbolicLink(at: symlinkURL, withDestinationURL: helperURL)

        XCTAssertEqual(TokenCueCLI.currentVersion(bundleVersion: nil, executablePath: symlinkURL.path), "2.4.6")
    }
    ...
    func test_cliVersionFindsAdjacentVersionWhenInvokedViaRelativePathAndSymlink() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-cli-version-invocation-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let executableURL = installURL.appendingPathComponent("TokenCueCLI")
        ...
        XCTAssertEqual(
            try Self.runVersionCommand(
                ...
                argv0: "install/bin/TokenCueCLI",
                currentDirectoryURL: workingDirectoryURL),
            "TokenCue 8.7.6\n")

        let symlinkURL = linksURL.appendingPathComponent("tokencue")
        try FileManager.default.createSymbolicLink(
            atPath: symlinkURL.path,
            withDestinationPath: "../install/bin/TokenCueCLI")
        XCTAssertEqual(
            try Self.runVersionCommand(
    ...
    func test_cliVersionPrefersAdjacentVersionOverStandaloneBundleName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-cli-version-bundle-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let binURL = root.appendingPathComponent("bin", isDirectory: true)
        try FileManager.default.createDirectory(at: binURL, withIntermediateDirectories: true)

        let helperURL = binURL.appendingPathComponent("TokenCueCLI")
        try Data().write(to: helperURL)
        try "4.5.6\n".write(
        ...
        XCTAssertEqual(
            TokenCueCLI.currentVersion(bundleVersion: "TokenCue", executablePath: helperURL.path),
            "4.5.6")
    }

    private func expectAdjacentVersionFile(raw: String, expected: String) throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-cli-version-file-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        XCTAssertEqual(TokenCueCLI.currentVersion(bundleVersion: nil, executablePath: helperURL.path), expected)
    }
        ...
        currentDirectoryURL: URL) throws -> String
    {
        ...
        process.environment = ProcessInfo.processInfo.environment.merging(
            ["TOKENCUE_SUPPRESS_TEST_KEYCHAIN_ACCESS": "1"]) { _, new in new }
        ...
        guard process.terminationStatus == 0 else {
            let message = String(bytes: errorOutput, encoding: .utf8)
                ?? "TokenCueCLI exited without an error message"
        ...
        guard let text = String(bytes: output, encoding: .utf8) else {
            throw NSError(domain: "CLIEntryTests", code: -1, userInfo: [
                NSLocalizedDescriptionKey: "TokenCueCLI produced non-UTF-8 output",
            ])
        }
        return text
    }
    ...
    private static var cliExecutableURL: URL {
        URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent(".build/debug/TokenCueCLI")
    }
    ...
    func test_renderOpenAIWebDashboardTextIncludesSummary() {
        ...
        let text = TokenCueCLI.renderOpenAIWebDashboardText(snapshot)
    ...
    func test_mapsErrorsToExitCodes() {
        XCTAssertEqual(TokenCueCLI.mapError(CodexStatusProbeError.codexNotInstalled), ExitCode(2))
        XCTAssertEqual(TokenCueCLI.mapError(CodexStatusProbeError.timedOut), ExitCode(4))
        XCTAssertEqual(TokenCueCLI.mapError(ClaudeWebFetchStrategyError.timedOut(seconds: 1)), ExitCode(4))
        XCTAssertEqual(TokenCueCLI.mapError(UsageError.noRateLimitsFound), ExitCode(3))
    }

    func test_antigravityPlanDebugKeepsOneShotHelperAliveUntilDebugFetch() {
        XCTAssertTrue(TokenCueCLI.holdsAntigravityCLISessionForPlanDebug(
            provider: .antigravity,
            planDebugEnabled: true,
            jsonOnly: false,
            persistsCLISessions: false))
        XCTAssertFalse(TokenCueCLI.holdsAntigravityCLISessionForPlanDebug(
            provider: .codex,
            planDebugEnabled: true,
            jsonOnly: false,
            persistsCLISessions: false))
    ...
    func test_missingCodexBinaryErrorPayloadUsesInstallGuidance() {
        let payload = TokenCueCLI.makeErrorPayload(CodexStatusProbeError.codexNotInstalled, kind: .provider)
    ...
    func test_providerSelectionFallsBackToBothForPrimaryPair() {
        let selection = TokenCueCLI.providerSelection(rawOverride: nil, enabled: [.codex, .claude])
        switch selection {
        case .both:
            break
        default:
    ...
    func test_providerSelectionFallsBackToCustomWhenNonPrimary() {
        let selection = TokenCueCLI.providerSelection(rawOverride: nil, enabled: [.codex, .gemini])
        switch selection {
    ...
    func test_providerSelectionHonorsEmptyEnabledSet() {
        let selection = TokenCueCLI.providerSelection(rawOverride: nil, enabled: [])
        switch selection {
        case let .custom(providers):
    ...
    func test_decodesSourceAndTimeoutOptions() throws {
        ...
        XCTAssertEqual(try TokenCueCLI._decodeWebTimeoutForTesting(from: parsed), 45)
        XCTAssertEqual(TokenCueCLI._decodeSourceModeForTesting(from: parsed), .oauth)

        let parsedWeb = try parser.parse(arguments: ["--web"])
        XCTAssertEqual(TokenCueCLI._decodeSourceModeForTesting(from: parsedWeb), .web)
    }
    ...
    func test_rejectsUnsafeWebTimeoutOptions() throws {
        for value in ["-1", "nan", "inf", "1e300"] {
            ...
            XCTAssertThrowsError(try TokenCueCLI._decodeWebTimeoutForTesting(from: parsed))
        }
    }

    func test_shouldUseColorRespectsFormatAndFlags() {
        XCTAssertFalse(TokenCueCLI.shouldUseColor(noColor: true, format: .text))
        XCTAssertFalse(TokenCueCLI.shouldUseColor(noColor: false, format: .json))
    }

    func test_kiloUsageTextNotesShowFallbackOnlyForAutoResolvedToCLI() {
        XCTAssertEqual(TokenCueCLI.usageTextNotes(
            provider: .kilo,
            sourceMode: .auto,
            resolvedSourceLabel: "cli"), ["Using CLI fallback"])
        XCTAssertTrue(TokenCueCLI.usageTextNotes(
            provider: .kilo,
            sourceMode: .api,
    ...
    func test_kiloAutoFallbackSummaryIncludesOrderedAttemptDetails() {
        ...
        let summary = TokenCueCLI.kiloAutoFallbackSummary(
            provider: .kilo,
            sourceMode: .auto,
            attempts: attempts)
        let expected = [
    ...
    func test_kiloAutoFallbackSummaryIsNilOutsideKiloAutoFailures() {
        ...
        XCTAssertNil(TokenCueCLI.kiloAutoFallbackSummary(
            provider: .kilo,
            sourceMode: .api,
            attempts: attempts))
        XCTAssertNil(TokenCueCLI.kiloAutoFallbackSummary(
    ...
    func test_sourceModeRequiresWebSupportIsProviderAware() throws {
        ...
        XCTAssertTrue(TokenCueCLI.sourceModeRequiresWebSupport(.web, provider: .kilo))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .codex))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .claude))
        XCTAssertTrue(TokenCueCLI.sourceModeRequiresWebSupport(.web, provider: .claude))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .kilo))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .grok))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.web, provider: .grok))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.auto, provider: .amp))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(.api, provider: .kilo))
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(
            .auto,
            provider: .opencodego,
        ...
        XCTAssertFalse(TokenCueCLI.sourceModeRequiresWebSupport(
            ...
            settings: ProviderSettingsSnapshot.make(
                opencodego: .init(
                    ...
                    workspaceID: nil))))
        XCTAssertTrue(TokenCueCLI.sourceModeRequiresWebSupport(
            .web,
            provider: .opencodego,
            settings: ProviderSettingsSnapshot.make(
                opencodego: .init(
