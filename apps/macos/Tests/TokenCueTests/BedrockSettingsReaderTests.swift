...
struct BedrockSettingsReaderTests {
    ...
    func `explicit profile auth mode wins`() {
        let env = ["TOKENCUE_BEDROCK_AUTH_MODE": "profile"]
    ...
    func `hasCredentials in profile mode requires a profile name`() {
        let withProfile = ["TOKENCUE_BEDROCK_AUTH_MODE": "profile", "AWS_PROFILE": "work"]
        let withoutProfile = ["TOKENCUE_BEDROCK_AUTH_MODE": "profile"]
