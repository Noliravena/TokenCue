...
struct CLIConfigCommandTests {
    ...
    func `Moonshot API key is bound to configured region`() {
        ...
        let updated = TokenCueCLI.configSettingAPIKey(
            config,
            provider: .moonshot,
            apiKey: "china-token",
            enableProvider: true)
    ...
    func `config set api key parses provider stdin and no enable flags`() throws {
        let parser = CommandParser(signature: TokenCueCLI._configSetAPIKeySignatureForTesting())
    ...
    func `config set api key for codex provider hints openai provider`() {
        ...
        let codexMsg = TokenCueCLI.unsupportedAPIKeyErrorMessage(for: .codex, rawProvider: "codex")
        #expect(codexMsg ==
            "codex does not support config API keys. For OpenAI Platform API keys, use '--provider openai'.")

        let claudeMsg = TokenCueCLI.unsupportedAPIKeyErrorMessage(for: .claude, rawProvider: "claude")
    ...
    func `config set api key stores key and enables provider`() {
        let config = TokenCueConfig.makeDefault()
        let updated = TokenCueCLI.configSettingAPIKey(
            config,
            provider: .elevenlabs,
    ...
    func `config set api key stores zai team token account`() throws {
        let config = TokenCueConfig.makeDefault()
        let options = try TokenCueCLI.resolveConfigAPIKeyAccountOptions(
            provider: .zai,
            label: "Team",
            usageScope: "team",
    ...
    func `config set api key rejects incomplete zai team account options`() {
        #expect(throws: CLIArgumentError.self) {
            _ = try TokenCueCLI.resolveConfigAPIKeyAccountOptions(
                provider: .zai,
                label: "Team",
                usageScope: "team",
                organizationID: "org-team",
    ...
    func `config provider toggle parses provider and json flags`() throws {
        let parser = CommandParser(signature: TokenCueCLI._configProviderToggleSignatureForTesting())
        let parsed = try parser.parse(arguments: [
            "--provider", "grok",
            "--json",
            "--pretty",
    ...
    func `config provider toggle enables and disables provider`() {
        let config = TokenCueConfig.makeDefault()
        let enabled = TokenCueCLI.configSettingProviderEnabled(config, provider: .grok, enabled: true)
        let disabled = TokenCueCLI.configSettingProviderEnabled(enabled, provider: .grok, enabled: false)
    ...
    func `config provider status includes effective default`() throws {
        let config = TokenCueConfig(providers: [
            ProviderConfig(id: .grok, enabled: true),
            ProviderConfig(id: .cursor, enabled: false),
        ])
        let statuses = TokenCueCLI.configProviderStatuses(config)
    ...
    func `config set api key rejects ambiguous input`() {
        #expect(throws: CLIArgumentError.self) {
            try TokenCueCLI.resolveConfigAPIKeyInput(apiKey: "xi-test-token", readFromStdin: true)
        }
    }

    @Test
    func `config help documents set api key`() {
        let help = TokenCueCLI.configHelp(version: "0.0.0")
    ...
    func `config dump parses show-secrets flag`() throws {
        let parser = CommandParser(signature: TokenCueCLI._configDumpSignatureForTesting())
    ...
    func `config dump redacts credentials by default`() {
        ...
        let config = TokenCueConfig(providers: [provider])
    ...
    func `config dump command redacts fixture secrets unless explicitly requested`() throws {
        let fixtureDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-config-dump-\(UUID().uuidString)", isDirectory: true)
        ...
        let config = TokenCueConfig(providers: [ProviderConfig(
            id: .zai,
            enabled: true,
            apiKey: secrets[0],
            secretKey: secrets[1],
    ...
    private static func runConfigDump(configURL: URL, showSecrets: Bool) throws -> Data {
        ...
        process.environment = ProcessInfo.processInfo.environment.merging([
            TokenCueConfigStore.pathEnvironmentKey: configURL.path,
            ...
            "TOKENCUE_SUPPRESS_TEST_KEYCHAIN_ACCESS": "1",
        ...
        guard process.terminationStatus == 0 else {
            let message = String(data: errorOutput, encoding: .utf8) ?? "TokenCueCLI exited without an error message"
