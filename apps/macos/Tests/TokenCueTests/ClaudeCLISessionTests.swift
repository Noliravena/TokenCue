...
struct ClaudeCLISessionTests {
    ...
    func `Claude session scope changes with account and config root and fails closed without identity`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-claude-scope-\(UUID().uuidString)", isDirectory: true)
    ...
    func `profile environment launches and identifies the reusable session`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-claude-environment-\(UUID().uuidString)", isDirectory: true)
        ...
        firstEnvironment["TOKENCUE_DISABLE_CLAUDE_WATCHDOG"] = "1"
    ...
    func `overlapping profile captures serialize PTY ownership`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-claude-overlap-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func `probe launch reuses one persisted session identifier`() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-claude-session-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        let file = directory.appendingPathComponent(".tokencue-session-id")
    ...
    func `unwritable probe directory keeps one process local fallback identifier`() {
        let directory = URL(fileURLWithPath: "/dev/null/TokenCue-ClaudeProbe", isDirectory: true)
