...
struct SpendDashboardModelTests {
    ...
    func `count labels avoid plural agreement and localize numbers`() {
        ...
        TokenCueLocalizationOverride.$appLanguage.withValue("fa") {
            #expect(tokenCueLocalizedInteger(12) == "۱۲")
    ...
    func `Codex account indices use app locale numerals`() throws {
        ...
        let persian = TokenCueLocalizationOverride.$appLanguage.withValue("fa") {
        ...
        let arabic = TokenCueLocalizationOverride.$appLanguage.withValue("ar") {
            SpendDashboardSource.codexRequest(
                account: account,
                homePath: home.path,
                providerName: "Codex",
