...
struct ClaudeOAuthUpgradeCompatibilityTests {
    ...
    @Test
    func `persisted OAuth uses TokenCue owned cache credentials only`() async throws {
        let root = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let cli = try Self.makeFakeClaudeCLI(in: root)
        let missingCredentials = root.appendingPathComponent("missing-credentials.json")
        let expectedToken = "tokencue-cache-oauth-token"
        ...
        case let .failure(error):
            Issue.record("Expected TokenCue cache OAuth fetch to succeed, got \(error)")
        }
    ...
    @Test
    func `app Auto preserves TokenCue owned cache OAuth before CLI and Web`() async throws {
        let root = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let cli = try Self.makeFakeClaudeCLI(in: root)
        let missingCredentials = root.appendingPathComponent("missing-credentials.json")
        let expectedToken = "auto-tokencue-cache-oauth-token"
        let environment = ["CLAUDE_CLI_PATH": cli.executable.path]
        let context = try self.makePersistedOAuthContext(
        ...
        case let .success(result):
            ...
            #expect(result.sourceLabel == "oauth")
        case let .failure(error):
            Issue.record("Expected Auto TokenCue cache OAuth fetch to succeed, got \(error)")
        }
        #expect(calls.recordedOAuthTokens == [expectedToken])
        #expect(calls.recordedWebCalls.isEmpty)
        #expect(calls.recordedForeignKeychainReads == 0)
    ...
    @Test
    func `invalid TokenCue OAuth cache remains terminal and does not switch authorities`() async throws {
        let root = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let cli = try Self.makeFakeClaudeCLI(in: root)
        let missingCredentials = root.appendingPathComponent("missing-credentials.json")
    ...
    #if os(macOS)
    @Test
    func `unavailable TokenCue OAuth cache remains terminal and does not switch authorities`() async throws {
        let root = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let cli = try Self.makeFakeClaudeCLI(in: root)
        let missingCredentials = root.appendingPathComponent("missing-credentials.json")
    ...
    @Test
    func `failed TokenCue cache OAuth remains terminal across refreshes`() async throws {
        let root = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let cli = try Self.makeFakeClaudeCLI(in: root)
        let missingCredentials = root.appendingPathComponent("missing-credentials.json")
    ...
    {
        let service = "com.tokencue.desktop.oauth-upgrade-tests.\(UUID().uuidString)"
