...
struct LocalizationBundleTests {
    ...
    func `packaged app resolves localization bundle from resources`() throws {
        ...
        let bundle = tokenCueLocalizationResourceBundle(mainBundle: fixture.appBundle)

        #expect(bundle.bundleURL.lastPathComponent == "TokenCue_TokenCue.bundle")
    ...
    func `empty localized values fall back to English`() throws {
        ...
        let resourceBundle = tokenCueLocalizationResourceBundle(mainBundle: fixture.appBundle)
        let zhPath = try #require(resourceBundle.path(forResource: "zh-Hans", ofType: "lproj"))
        let zhBundle = try #require(Bundle(path: zhPath))

        #expect(tokenCueLocalizedString("Settings", bundle: zhBundle, resourceBundle: resourceBundle) == "Settings")
        #expect(tokenCueLocalizedString("Missing", bundle: zhBundle, resourceBundle: resourceBundle) == "Missing")
    }
        ...
        includeEmptyChineseLocalization: Bool = false) throws -> (root: URL, appBundle: Bundle)
    {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "tokencue-localization-\(UUID().uuidString)",
            isDirectory: true)
        ...
        <plist version="1.0">
        <dict>
            <key>CFBundleExecutable</key><string>TokenCue</string>
            <key>CFBundleIdentifier</key><string>com.tokencue.desktop.tests</string>
            <key>CFBundleName</key><string>TokenCue</string>
        ...
        if includeLocalizationBundle {
            let bundleURL = resourcesURL.appendingPathComponent("TokenCue_TokenCue.bundle", isDirectory: true)
