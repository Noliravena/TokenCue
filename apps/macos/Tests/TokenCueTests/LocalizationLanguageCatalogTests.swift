...
struct LocalizationLanguageCatalogTests {
    ...
    func `adaptive activity consent is localized in every app language`() throws {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let resourcesURL = root.appendingPathComponent("Sources/TokenCue/Resources")
        let keys = [
    ...
    func `language picker labels use stable native names`() {
        ...
        let japaneseLabels = TokenCueLocalizationOverride.$appLanguage.withValue("ja") {
            Dictionary(uniqueKeysWithValues: AppLanguage.allCases.map { ($0, $0.label) })
        }
        let arabicLabels = TokenCueLocalizationOverride.$appLanguage.withValue("ar") {
            Dictionary(uniqueKeysWithValues: AppLanguage.allCases.map { ($0, $0.label) })
        }
    ...
    func `german manual action labels do not describe a handbook`() throws {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let germanURL = root.appendingPathComponent("Sources/TokenCue/Resources/de.lproj/Localizable.strings")
    ...
    func `ukrainian localization bundle exists and contains key UI labels`() throws {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let ukURL = root.appendingPathComponent("Sources/TokenCue/Resources/uk.lproj/Localizable.strings")
    ...
    func `korean localization bundle includes representative native labels`() throws {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let koURL = root.appendingPathComponent("Sources/TokenCue/Resources/ko.lproj/Localizable.strings")
        ...
        #expect(catalog["quit_app"] == "TokenCue 종료")
    }
    ...
    func `turkish localization matches English catalog and preserves format placeholders`() throws {
        ...
        #expect(turkish["quit_app"] == "TokenCue'dan Çık")
    ...
    func `indonesian localization matches English catalog and preserves format placeholders`() throws {
        ...
        #expect(indonesian["quit_app"] == "Keluar TokenCue")
    ...
    func `japanese usage chart accessibility text preserves argument meanings`() throws {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let jaURL = root.appendingPathComponent("Sources/TokenCue/Resources/ja.lproj/Localizable.strings")
