...
final class ProviderVersionDetectorTests: XCTestCase {
    ...
    func test_run_returnsOutputWhenDetachedChildKeepsPipeOpen() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-version-drain-\(UUID().uuidString)", isDirectory: true)
        ...
        (trap '' HUP; sleep 5) &
        child=$!
        printf '%s' "$child" > "$TOKENCUE_TEST_CHILD_PID_FILE"
        printf 'grok 1.2.3\\n'
        """
    ...
    {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tokencue-version-profile-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
