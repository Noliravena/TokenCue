...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "tokencueLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("tokencue-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__tokencueAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.tokencueLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__tokencueNextPolling) return;
        window.__tokencueNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__tokencueNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__tokencueNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__tokencueNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__tokencueAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = TokenCueLog.logger(LogCategories.creditsPurchase)
