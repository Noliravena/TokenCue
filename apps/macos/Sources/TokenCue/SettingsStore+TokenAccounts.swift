...
extension SettingsStore {
    ...
    func reloadTokenAccounts() {
        let log = TokenCueLog.logger(LogCategories.tokenAccounts)
    ...
    func openTokenAccountsFile() {
        do {
            ...
            TokenCueLog.logger(LogCategories.tokenAccounts).error("Failed to persist config: \(error)")
            return
        }
        ...
        removedCredentials: AntigravityOAuthCredentials)
    {
        do {
            ...
            TokenCueLog.logger(LogCategories.tokenAccounts).warning(
