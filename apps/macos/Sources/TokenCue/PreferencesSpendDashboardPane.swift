...
func spendDashboardDayRangeText(_ days: Int) -> String {
    ...
    default: return tokenCueLocalizedInteger(days)
    }
    return template.replacingOccurrences(
        of: String(days),
        with: tokenCueLocalizedInteger(days))
}

func spendDashboardRankText(_ rank: Int) -> String {
    "#\(tokenCueLocalizedInteger(rank))"
}

func spendDashboardRefreshFailureText(_ count: Int) -> String {
    "\(L("Refresh failures")): \(tokenCueLocalizedInteger(count))"
}

func spendDashboardCoverageText(covered: Int, requested: Int) -> String {
    "\(L("Coverage")): \(tokenCueLocalizedInteger(covered)) / \(tokenCueLocalizedInteger(requested))"
}
...
func codexCostCatchUpProgressText(_ activity: CodexCostCatchUpActivity) -> String {
    ...
    if activity.totalFiles > 0 {
        return "\(tokenCueLocalizedInteger(activity.completedFiles)) / "
            + tokenCueLocalizedInteger(activity.totalFiles)
    }
    return L("Loading…")
}
            ...
            SpendDashboardPanel {
                HStack(spacing: 24) {
                    ...
                    SpendSummaryValue(
                        ...
                        value: tokenCueLocalizedInteger(self.group.providers.count))
                    Spacer()
                }
            }
...
private struct SpendDailyChart: View {
    ...
    private func pointAccessibilityLabel(_ point: SpendDashboardModel.DailyPoint) -> String {
        let day = point.day.formatted(
            .dateTime.month(.abbreviated).day().locale(tokenCueLocalizedLocale()))
