...
final class StatusItemController: NSObject, NSMenuDelegate, StatusItemControlling {
    ...
    private nonisolated static let statusItemAccessibilityTitle = "TokenCue"
    private nonisolated static let debugStatusItemAccessibilityTitle = "TokenCue Debug"
    private nonisolated static let statusItemAccessibilityIdentifierPrefix = "TokenCue.StatusItem"
    ...
    lazy var tokenCuePanelController = TokenCuePanelController(
        store: self.store,
    ...
    let loginLogger = TokenCueLog.logger(LogCategories.login)
    let menuLogger = TokenCueLog.logger(LogCategories.app)
    ...
    @objc private func handleProviderConfigDidChange(_ notification: Notification) {
        ...
        {
            if let config = notification.userInfo?["config"] as? TokenCueConfig {
    ...
    private func attachMenus() {
        if !SettingsStore.isRunningTests {
            self.installTokenCuePanelAction(on: self.statusItem)
            return
        }
    ...
    private func attachMenus(fallback: UsageProvider? = nil) {
        if !SettingsStore.isRunningTests {
            for provider in UsageProvider.allCases {
                ...
                if shouldHaveItem {
                    self.installTokenCuePanelAction(on: self.lazyStatusItem(for: provider))
