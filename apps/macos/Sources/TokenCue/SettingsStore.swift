...
final class SettingsStore {
    /// TokenCue has no app-group, widget, iCloud, or cross-device defaults bridge.
    static let sharedDefaults: UserDefaults? = nil
    ...
    @ObservationIgnored let configStore: TokenCueConfigStore
    @ObservationIgnored let antigravityOAuthCredentialsStore: AntigravityOAuthCredentialsStore
    @ObservationIgnored var config: TokenCueConfig
    ...
    /// Installed/login sources discovered without reading browser Cookie data.
    /// Onboarding presents these as suggestions but never enables them automatically.
    var detectedProviderCandidates: Set<UsageProvider> = []
    ...
    init(
        ...
        configStore: TokenCueConfigStore = TokenCueConfigStore(),
        ...
        performInitialProviderDetection: Bool = !SettingsStore.isRunningTests)
    {
        ...
        let config: TokenCueConfig
        do {
            config = try configStore.load() ?? .makeDefault()
        } catch {
            TokenCueLog.logger(LogCategories.configStore).error(
                "Failed to load TokenCue config; using fresh defaults: \(error)")
            config = .makeDefault()
        }
        ...
        TokenCueLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
        ...
        hadPreviousInstallationState: Bool) -> SettingsDefaultsState
    {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? TokenCueLog.Level.verbose.rawValue
        ...
        // TokenCue keeps costs in each provider's native currency. The retained
        // state field is compatibility-only for inherited view models.
        let preferredCurrencyCode = "auto"
...
extension SettingsStore {
    var configSnapshot: TokenCueConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: TokenCueConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        TokenCueLog.logger(LogCategories.settings).debug(
