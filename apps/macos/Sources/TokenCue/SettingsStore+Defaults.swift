...
extension SettingsStore {
    ...
    var isVerboseLoggingEnabled: Bool {
        self.debugLogLevel.rank <= TokenCueLog.Level.verbose.rank
    }
    ...
    var copilotBudgetExtrasEnabled: Bool {
        ...
        set {
            ...
            TokenCueLog.logger(LogCategories.settings).info(
    ...
    var preferredCurrencyCode: String {
        get { "auto" }
        set {
            // Cross-currency conversion is intentionally outside the shared
            // TokenCue product contract. Preserve API compatibility while
            // forcing provider-native currency everywhere in the app.
            self.defaultsState.preferredCurrencyCode = "auto"
            self.userDefaults.removeObject(forKey: "preferredCurrencyCode")
        }
    }
}
