...
struct TokenCueConfigMigrator {
    ...
    static func loadOrMigrate(
        ...
        stores: LegacyStores) -> TokenCueConfig
    {
        let log = TokenCueLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? TokenCueConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: TokenCueConfig) -> TokenCueConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: TokenCueConfig) -> TokenCueConfig
    {
        var updated = config
