...
extension SettingsStore {
    ...
    func applyProviderDetection() async {
        ...
        let logger = TokenCueLog.logger(LogCategories.providerDetection)

        let detectedProviders = ProviderDetectionPolicy.enabledProviders(signals: .init(
        ...
        logger.info(
            ...
            metadata: [
                "codex": detectedProviders.contains(.codex) ? "1" : "0",
                "claude": detectedProviders.contains(.claude) ? "1" : "0",
                "gemini": detectedProviders.contains(.gemini) ? "1" : "0",
                "antigravity": detectedProviders.contains(.antigravity) ? "1" : "0",
            ])
        self.detectedProviderCandidates = detectedProviders
        self.providerDetectionCompleted = true
        logger.info("Provider detection completed without enabling providers or reading browser cookies")
    }
}
