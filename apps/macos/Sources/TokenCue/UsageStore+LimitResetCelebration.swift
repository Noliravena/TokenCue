...
extension UsageStore {
        ...
        observation: LimitResetObservation?)
    {
        ...
        if claudeWeeklyRecoveryPending, wasAboveThreshold {
            TokenCueLog.logger(LogCategories.confetti).debug(
        ...
        TokenCueLog.logger(LogCategories.confetti).info(
        ...
        case .session:
            ...
            let event = SessionLimitResetEvent(
                ...
                accountLabel: accountLabel,
                usedPercent: currentUsed)
            NotificationCenter.default.post(name: .tokencueSessionLimitReset, object: event)
        case .weekly:
            self.emitQuotaResetHook(
                provider: context.provider,
                window: .weekly,
            ...
            let event = WeeklyLimitResetEvent(
                ...
                accountLabel: accountLabel,
                usedPercent: currentUsed)
            NotificationCenter.default.post(name: .tokencueWeeklyLimitReset, object: event)
        default:
            return
        }
    }
