...
private enum KeychainPromptMessage {
    static let browserCookie =
        "TokenCue will ask macOS Keychain for “%@” so it can decrypt browser cookies " +
        "and authenticate your account. Click OK to continue."

    static let claudeOAuth =
        "TokenCue will ask macOS Keychain for the Claude Code OAuth token " +
        "so it can fetch your Claude usage. Click OK to continue."
    static let codexCookie =
        "TokenCue will ask macOS Keychain for your OpenAI cookie header " +
        "so it can fetch Codex dashboard extras. Click OK to continue."
    static let claudeCookie =
        "TokenCue will ask macOS Keychain for your Claude cookie header " +
        "so it can fetch Claude web usage. Click OK to continue."
    static let cursorCookie =
        "TokenCue will ask macOS Keychain for your Cursor cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let openCodeCookie =
        "TokenCue will ask macOS Keychain for your OpenCode cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let factoryCookie =
        "TokenCue will ask macOS Keychain for your Factory cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let zaiToken =
        "TokenCue will ask macOS Keychain for your z.ai API token " +
        "so it can fetch usage. Click OK to continue."
    static let syntheticToken =
        "TokenCue will ask macOS Keychain for your Synthetic API key " +
        "so it can fetch usage. Click OK to continue."
    static let copilotToken =
        "TokenCue will ask macOS Keychain for your GitHub Copilot token " +
        "so it can fetch usage. Click OK to continue."
    static let kimiToken =
        "TokenCue will ask macOS Keychain for your Kimi auth token " +
        "so it can fetch usage. Click OK to continue."
    static let minimaxCookie =
        "TokenCue will ask macOS Keychain for your MiniMax cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let minimaxToken =
        "TokenCue will ask macOS Keychain for your MiniMax API token " +
        "so it can fetch usage. Click OK to continue."
    static let augmentCookie =
        "TokenCue will ask macOS Keychain for your Augment cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let ampCookie =
        "TokenCue will ask macOS Keychain for your Amp cookie header " +
        "so it can fetch usage. Click OK to continue."
}
...
enum KeychainPromptCoordinator {
    ...
    private static let log = TokenCueLog.logger(LogCategories.keychainPrompt)
    private static let documentationURL =
        "https://github.com/Noliravena/TokenCue/blob/main/docs/keychain-prompts.md"
    ...
    static func disableKeychainForUnbundledExecutableIfNeeded() {
        ...
        guard Self.isUnbundledTokenCueExecutable(executablePath) else { return }
        KeychainAccessGate.forceDisabledForProcess(reason: "unbundled-executable")
        Self.log.warning(
            "Unbundled TokenCue executable detected; disabling keychain access to avoid repeated prompts",
            metadata: ["doc": "docs/DEVELOPMENT_SETUP.md"])
    }

    static func isUnbundledTokenCueExecutable(_ executablePath: String) -> Bool {
        guard executablePath.hasPrefix("/") else { return false }
        let executableURL = URL(fileURLWithPath: executablePath).standardizedFileURL
        return executableURL.lastPathComponent == "TokenCue"
