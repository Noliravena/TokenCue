...
extension StatusItemController: StatusItemMenuPersistentActionDelegate {
    ...
    @objc func addManagedCodexAccountFromMenu(_: NSMenuItem) {
        ...
        guard self.settings.hasUnreadableManagedCodexAccountStore == false else {
            self.presentLoginAlert(
                ...
                message: L(
                    "TokenCue could not read managed account storage. " +
    ...
    func openMenuFromShortcut() {
        if !SettingsStore.isRunningTests {
            let item = self.shouldMergeIcons
                ? self.statusItem
                : self.lazyStatusItem(for: self.resolvedShortcutProvider())
            guard let button = item.button else { return }
            self.tokenCuePanelController.toggle(relativeTo: button)
            return
        }
    ...
    func closeOpenMenusFromShortcutIfNeeded() -> Bool {
        if !SettingsStore.isRunningTests, self.tokenCuePanelController.isVisible {
            self.tokenCuePanelController.close()
            return true
        }
    ...
    func openTerminal(command: String) {
        ...
        if terminal == .iTerm, !terminal.isInstalled {
            TokenCueLog.logger(LogCategories.terminal).warning(
    ...
    private static func executeAppleScript(_ source: String) -> Bool {
        if let appleScript = NSAppleScript(source: source) {
            ...
            if let error {
                TokenCueLog.logger(LogCategories.terminal).error(
        ...
        TokenCueLog.logger(LogCategories.terminal).error("Failed to compile AppleScript")
        return false
    }
