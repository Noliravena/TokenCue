...
enum TokenCueLocalizationOverride {
...
private func appLanguageDefaults() -> UserDefaults {
    ...
    return UserDefaults(suiteName: "TokenCue") ?? .standard
}
...
private func resolvedAppLanguage() -> String {
    if let override = TokenCueLocalizationOverride.appLanguage {
        return override
    }
...
func tokenCueLocalizationSignature() -> String {
    resolvedAppLanguage()
}

/// Resolving the `.lproj`/resource bundles repeats `Bundle(url:)`/`Bundle(path:)` filesystem lookups,
/// which are surprisingly hot: every `L(…)` and `tokenCueLocalizationSignature()` call runs them, and
...
func tokenCueLocalizationResourceBundle(
    mainBundle: Bundle = .main,
    bundleName: String = "TokenCue_TokenCue") -> Bundle
{
    ...
    guard mainBundle === Bundle.main, bundleName == "TokenCue_TokenCue" else {
...
private func resolveLocalizedBundle(forLanguage language: String) -> Bundle {
    let resourceBundle = tokenCueLocalizationResourceBundle()
    if !language.isEmpty {
...
func L(_ key: String) -> String {
    let resourceBundle = tokenCueLocalizationResourceBundle()
    return tokenCueLocalizedString(key, bundle: localizedBundle(), resourceBundle: resourceBundle)
}
...
func L(_ key: String, language: String) -> String {
    ...
    return tokenCueLocalizedString(key, bundle: bundle, resourceBundle: resourceBundle)
}

func tokenCueLocalizedLocale() -> Locale {
    tokenCueLocale(forLanguage: resolvedAppLanguage())
}
...
func tokenCueLocalizedResourceLocale() -> Locale {
    let bundleURL = localizedBundle().bundleURL
    guard bundleURL.pathExtension == "lproj" else {
        return tokenCueLocalizedLocale()
    }
    return tokenCueLocale(forLanguage: bundleURL.deletingPathExtension().lastPathComponent)
}

private func tokenCueLocale(forLanguage language: String) -> Locale {
...
func tokenCueLocalizedInteger(_ value: Int) -> String {
    value.formatted(.number.locale(tokenCueLocalizedLocale()))
}

func tokenCueLocalizedString(_ key: String, bundle: Bundle, resourceBundle: Bundle) -> String {
...
func resetTokenCueLocalizationCache() {
    LocalizationBundleCache.reset()
}

#if DEBUG
func tokenCueLocalizedBundleForTesting() -> Bundle {
    localizedBundle()
}

func resetTokenCueLocalizationCacheForTesting() {
