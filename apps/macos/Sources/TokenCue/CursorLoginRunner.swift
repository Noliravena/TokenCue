...
final class CursorLoginRunner {
    ...
    private let logger = TokenCueLog.logger(LogCategories.provider(.cursor, scope: "login"))
    ...
    private func timeoutMessage(lastError: Error?) -> String {
        ...
        let hint = L("Sign in to cursor.com in your browser, then refresh Cursor in TokenCue.")
        guard let lastError else {
