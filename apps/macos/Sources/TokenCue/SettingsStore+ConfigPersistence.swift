...
extension SettingsStore {
    ...
    func updateConfig(
        ...
        mutate: (inout TokenCueConfig) -> Void)
    {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        TokenCueLog.logger(LogCategories.tokenAccounts).info(
        ...
        origin: ConfigChangeOrigin = .externalSync)
    {
        ...
        do {
            ...
            TokenCueLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(
        _ config: TokenCueConfig,
        reason: String,
        affectsBackgroundWork: Bool? = nil,
        origin: ConfigChangeOrigin = .externalSync)
    {
    ...
    private static func configChangeAffectsBackgroundWork(
        from previous: TokenCueConfig,
        to current: TokenCueConfig) -> Bool
    {
    ...
    private static func orderIndependentConfigData(_ config: TokenCueConfig) -> Data? {
    ...
    private func bumpConfigRevision(_ context: ConfigChangeContext) {
        ...
        TokenCueLog.logger(LogCategories.settings)
            .debug(
        ...
        if context.shouldNotifyCloudSync {
            NotificationCenter.default.post(
                name: .tokencueLocalConfigFileDidChange,
                object: self,
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                TokenCueLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
        ...
        self.configPersistTask = Task { @MainActor in
            ...
            do {
                ...
                TokenCueLog.logger(LogCategories.configStore).error("Failed to encode config: \(error)")
                return
            }
