...
enum MenuBarVisibilityWatcher {
        ...
        openURL: (URL) -> Void = { NSWorkspace.shared.open($0) })
    {
        ...
        let alert = NSAlert()
        alert.messageText = L("TokenCue can't show its menu bar icon")
        alert.informativeText = L(
            "macOS Tahoe can block menu bar apps in System Settings → Menu Bar → Allow in the Menu Bar. "
                + "TokenCue is running, but macOS may be hiding its icon. Open Menu Bar settings and turn TokenCue on.")
