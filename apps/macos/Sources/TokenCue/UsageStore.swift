...
final class UsageStore {
    ...
    @ObservationIgnored let sessionQuotaLogger = TokenCueLog.logger(LogCategories.sessionQuota)
    // Provider-specific by design: OpenAI web and Augment runtime diagnostics have dedicated app-owned log streams.
    @ObservationIgnored let openAIWebLogger = TokenCueLog.logger(LogCategories.provider(.openai, scope: "web"))
    @ObservationIgnored private let tokenCostLogger = TokenCueLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = TokenCueLog.logger(LogCategories.provider(.augment))
    @ObservationIgnored let providerLogger = TokenCueLog.logger(LogCategories.providers)
    @ObservationIgnored let adaptiveRefreshLogger = TokenCueLog.logger(LogCategories.adaptiveRefresh)
    ...
    var codexBrowserCookieOrder: BrowserCookieImportOrder {
        self.metadata(for: .codex).browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
    }
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("tokencue-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "tokencue-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: TokenCueCore.ProviderRuntime.app,
