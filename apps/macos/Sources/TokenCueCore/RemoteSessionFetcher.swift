...
public struct RemoteSessionFetcher: Sendable {
    public static let bundledCLIFallback = "/Applications/TokenCue.app/Contents/Helpers/TokenCueCLI"
        ...
        environment: [String: String] = ProcessInfo.processInfo.environment) async
    {
        ...
        let command = "tokencue sessions focus \(Self.shellQuote(sessionID)) || " +
    ...
    /// wrapper, but a GUI-launched TokenCue inherits a minimal `PATH` (`/usr/bin:/bin`)
