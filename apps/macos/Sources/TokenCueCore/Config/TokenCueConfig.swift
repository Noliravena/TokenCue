...
public struct TokenCueConfig: Codable, Sendable {
    public static let currentVersion = 1

    private static let log = TokenCueLog.logger(LogCategories.configStore)
    ...
    public static func makeDefault(
        metadata: [UsageProvider: ProviderMetadata] = ProviderDescriptorRegistry.metadata) -> TokenCueConfig
    {
        ...
        return TokenCueConfig(version: Self.currentVersion, providers: providers)
    }
        ...
        metadata: [UsageProvider: ProviderMetadata] = ProviderDescriptorRegistry.metadata) -> TokenCueConfig
    {
        ...
        return TokenCueConfig(
            version: Self.currentVersion,
            providers: normalized,
            hooks: self.hooks)
    }

    public func sanitizedForDump(showSecrets: Bool = false) -> TokenCueConfig {
