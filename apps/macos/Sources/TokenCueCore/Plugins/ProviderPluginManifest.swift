...
public struct ProviderPluginLimits: Equatable, Sendable {
    ...
    public init(
        ...
        maximumResponseBytes: Int = Self.defaultResponseBytes)
    {
...
public struct ProviderPluginManifest: @unchecked Sendable {
    ...
    public let limits: ProviderPluginLimits
    ...
    init(definition: JSValue, allowsDynamicID: Bool = false) throws {
        ...
        if let version = definition.forProperty("schemaVersion"), !version.isUndefined, !version.isNull {
            guard version.isNumber, version.toDouble() == 1 else {
                throw ProviderPluginError.invalidManifest("schemaVersion must be 1")
            }
        }
        ...
        let limitsValue = definition.forProperty("limits")
        if let limitsValue, !limitsValue.isUndefined, !limitsValue.isNull {
            guard limitsValue.isObject, !limitsValue.isArray else {
                throw ProviderPluginError.invalidManifest("'limits' must be an object when present")
            }
            let timeoutMilliseconds = try Self.optionalBoundedInteger(
                limitsValue,
                property: "timeoutMs",
                defaultValue: ProviderPluginLimits.defaultTimeoutMilliseconds,
                minimum: 100,
                maximum: ProviderPluginLimits.maximumTimeoutMilliseconds)
            let maximumResponseBytes = try Self.optionalBoundedInteger(
                limitsValue,
                property: "maxResponseBytes",
                defaultValue: ProviderPluginLimits.defaultResponseBytes,
                minimum: 1024,
                maximum: ProviderPluginLimits.maximumResponseBytes)
            self.limits = ProviderPluginLimits(
    ...
    private static func optionalBoundedInteger(
        _ object: JSValue,
        property: String,
        defaultValue: Int,
        minimum: Int,
    ...
    {
        guard let value = object.forProperty(property), !value.isUndefined, !value.isNull else {
            return defaultValue
        }
        guard value.isNumber else {
            throw ProviderPluginError.invalidManifest("'\(property)' must be an integer")
        }
