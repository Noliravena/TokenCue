...
enum ProviderPluginCookieBroker {
    ...
    {
        #if os(macOS)
        let importOrder = ProviderDefaults.metadata[provider]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
        ...
        for browser in importOrder.cookieImportCandidates(using: browserDetection) {
            do {
                let sources = try client.tokenCueRecords(matching: query, in: browser)
