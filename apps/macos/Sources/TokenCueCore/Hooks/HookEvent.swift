...
/// The quota/provider state changes TokenCue can run external commands for.
///
...
public struct HookEvent: Codable, Sendable, Equatable {
    ...
    public func environmentVariables() -> [String: String] {
        var env: [String: String] = [
            "TOKENCUE_EVENT": self.event.rawValue,
            "TOKENCUE_PROVIDER": self.provider,
            "TOKENCUE_TIMESTAMP": Self.iso8601String(self.timestamp),
        ]
        if let account { env["TOKENCUE_ACCOUNT"] = account }
        if let window { env["TOKENCUE_WINDOW"] = window }
        if let usagePercent { env["TOKENCUE_USAGE_PERCENT"] = Self.number(usagePercent) }
        if let used { env["TOKENCUE_USED"] = Self.number(used) }
        if let limit { env["TOKENCUE_LIMIT"] = Self.number(limit) }
        if let resetAt { env["TOKENCUE_RESET_AT"] = Self.iso8601String(resetAt) }
        if let status { env["TOKENCUE_STATUS"] = status }
        return env
    }
