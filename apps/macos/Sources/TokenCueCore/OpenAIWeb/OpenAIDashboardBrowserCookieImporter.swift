...
public struct OpenAIDashboardBrowserCookieImporter {
    ...
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.codex]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
        ...
        do {
            ...
            do {
                sources = try await Self.runBoundedCookieLoad(deadline: deadline) {
                    try Self.cookieClient.tokenCueRecords(matching: query, in: .safari)
                }
        ...
        diagnostics: inout ImportDiagnostics) async throws -> ImportResult?
    {
        do {
            ...
            do {
                sources = try await Self.runBoundedCookieLoad(deadline: deadline) {
                    try Self.cookieClient.tokenCueRecords(matching: query, in: browser)
                }
            } catch let error as URLError where error.code == .timedOut {
