...
let openAIDashboardScrapeScript = """
    ...
    (() => {
      ...
      const usageBreakdownJSON = (() => {
        try {
          if (window.__tokencueUsageBreakdownJSON) return window.__tokencueUsageBreakdownJSON;
          ...
          window.__tokencueUsageBreakdownJSON = json;
          window.__tokencueUsageBreakdownDebug = json ? null : JSON.stringify(debug);
          return json;
        } catch {
          return null;
        }
      ...
      const usageBreakdownDebug = (() => {
        try {
          return window.__tokencueUsageBreakdownDebug || null;
        } catch {
          return null;
        }
      })();
      ...
      try {
        ...
        if (header) {
          ...
          if (rows.length === 0 && !window.__tokencueDidScrollToCredits) {
            window.__tokencueDidScrollToCredits = true;
        ...
        } else if (rows.length === 0 && !window.__tokencueDidScrollToCredits && scrollHeight > viewportHeight * 1.5) {
          rows = allTableRows();
          if (rows.length > 0) {
            creditsHeaderPresent = true;
        ...
        if (rows.length === 0 && !window.__tokencueDidScrollToCredits && scrollHeight > viewportHeight * 1.5) {
