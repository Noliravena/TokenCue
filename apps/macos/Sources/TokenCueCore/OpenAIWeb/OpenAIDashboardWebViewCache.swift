...
final class OpenAIDashboardWebViewCache {
    ...
    fileprivate static let log = TokenCueLog.logger(LogCategories.provider(.openai, scope: "webview"))
    ...
    (() => {
      try {
        delete window.__tokencueDidScrollToCredits;
        delete window.__tokencueUsageBreakdownJSON;
        delete window.__tokencueUsageBreakdownDebug;
        return true;
      } catch {
        return false;
      }
