...
/// are passed back to TokenCue.
let openAISubscriptionCaptureScript = """
(() => {
  if (window.__tokencueSubscriptionCaptureInstalled) return;
  window.__tokencueSubscriptionCaptureInstalled = true;
  window.__tokencueSubscriptionCaptureGeneration = 0;
  const originalFetch = window.fetch.bind(window);
  window.fetch = async (...args) => {
    const generation = window.__tokencueSubscriptionCaptureGeneration;
    ...
    try {
      ...
      if (requestURL.origin === window.location.origin &&
        ...
        response.clone().json().then(payload => {
          if (window.__tokencueSubscriptionCaptureGeneration !== generation) return;
          window.__tokencueSubscriptionMetadata = {
...
(() => {
  const generation = Number(window.__tokencueSubscriptionCaptureGeneration || 0) + 1;
  window.__tokencueSubscriptionCaptureGeneration = generation;
  window.__tokencueSubscriptionMetadata = null;
  return generation;
})();
"""
...
(() => ({
  ...
  metadata: window.__tokencueSubscriptionMetadata || null
}))();
"""
