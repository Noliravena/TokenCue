...
/// process. ``agy`` is an interactive REPL, not a query command, so TokenCue
...
struct AntigravityCLIHTTPSFetchStrategy: ProviderFetchStrategy {
    ...
    private static let log = TokenCueLog.logger(LogCategories.provider(.antigravity))
    ...
    struct WarmAgyDependencies {
        ...
        /// The pid of an ``agy`` that TokenCue itself spawned and manages through
        ...
        /// long-lived `agy`, or another TokenCue host) has no such accounting.
        ...
        dependencies: WarmAgyDependencies) async throws -> AntigravityStatusSnapshot?
    {
        ...
        // Also exclude any `agy` TokenCue itself spawned and manages: reusing it
    ...
    static func liveWarmAgyDependencies() -> WarmAgyDependencies {
        WarmAgyDependencies(
            ...
            ownedPID: {
                // The pid of the `agy` TokenCue manages through the shared
        ...
        async throws -> ProviderFetchResult
    {
        ...
        // process (an IDE, a long-lived `agy`, or another TokenCue host), so
        // TokenCue must not manage its lifecycle, idle timeout, or
        // `resetAfterFetch` teardown. Those apply only to processes TokenCue
    ...
    /// Spawn (or reuse TokenCue's own warm) `agy` session and wait for the CLI
    ...
    static func shouldResetSessionAfterFetch(_ context: ProviderFetchContext) -> Bool {
        // Long-lived hosts (the app, `tokencue serve`) keep the warm `agy`
