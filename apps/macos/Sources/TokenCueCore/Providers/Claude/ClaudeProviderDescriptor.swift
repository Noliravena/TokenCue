...
public enum ClaudeProviderDescriptor {
    ...
    private static let ttyLaunch = ProviderTTYLaunchConfig(
        ...
        bundledWatchdogHelperName: "TokenCueClaudeWatchdog",
        ...
        environment: [String: String]) -> Bool
    {
        ...
        if let nonInteractiveRecord, hasRequiredScopeWithoutPrompt, nonInteractiveRecord.credentials.isExpired {
            ...
            case .claudeCLI:
                ...
                // That child process can access Keychain outside TokenCue's no-UI controls, so do
...
struct ClaudeCLIFetchStrategy: ProviderFetchStrategy {
    ...
    func isAvailable(_ context: ProviderFetchContext) async -> Bool {
        ...
        // TokenCue cannot impose its no-UI policy on that child, so background Auto refresh must not launch it
        ...
        if isBackgroundAutoRefresh {
            // Every Claude child process is opaque to TokenCue's no-UI Keychain controls, including
