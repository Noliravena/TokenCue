...
actor ClaudeCLISession {
    ...
    private static let log = TokenCueLog.logger(LogCategories.provider(.claude, scope: "cli"))
    private static let probeSessionIDFilename = ".tokencue-session-id"
    ...
    enum SessionError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .outputTooLarge: "Claude CLI session produced more output than TokenCue can safely process."
            }
        }
    }
        ...
        environment: [String: String]) throws
    {
        ...
        let disableWatchdog = sessionIdentity.environment["TOKENCUE_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
           resolvedURL.lastPathComponent == "claude",
           let watchdog = TTYCommandRunner.locateBundledHelper("TokenCueClaudeWatchdog")
        {
