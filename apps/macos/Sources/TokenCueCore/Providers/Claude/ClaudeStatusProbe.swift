...
public struct ClaudeStatusProbe: Sendable {
    ...
    private static let log = TokenCueLog.logger(LogCategories.provider(.claude, scope: "probe"))
    #if DEBUG
...
extension ClaudeStatusProbe {
    ...
    private static func extractUsageError(text: String) -> String? {
        ...
        if lower.contains("do you trust the files in this folder?"), !lower.contains("current session") {
            ...
            if let folderHint {
                return """
                Claude CLI is waiting for a folder trust prompt (\(folderHint)). TokenCue tries to auto-accept this, \
                but if it keeps appearing run: `cd "\(folderHint)" && claude` and choose “Yes, proceed”, then retry.
                """
            }
            return """
            Claude CLI is waiting for a folder trust prompt. TokenCue tries to auto-accept this, but if it keeps \
