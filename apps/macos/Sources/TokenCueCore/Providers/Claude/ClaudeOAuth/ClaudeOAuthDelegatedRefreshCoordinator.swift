...
public enum ClaudeOAuthDelegatedRefreshCoordinator {
    ...
    /// `Outcome` ships in the `TokenCueCore` library product, so a new case would break downstream exhaustive
        ...
        state: AttemptStateStorage) async -> AttemptResult
    {
        ...
        // TokenCue's own no-UI query controls. Background work may not cross that boundary unless
