...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "TOKENCUE_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "TOKENCUE_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "TOKENCUE_CLAUDE_OAUTH_CLIENT_ID"
    ...
    static let log = TokenCueLog.logger(LogCategories.provider(.claude, scope: "usage"))
    ...
    private static let pendingTokenCueOAuthKeychainCacheClearKey =
        "ClaudeOAuthPendingTokenCueOAuthKeychainCacheClearV1"
    private static let pendingTokenCueOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore =
        ClaudeOAuthPendingCacheClearUserDefaultsStore(
            ...
            domain: "com.tokencue.desktop",
            key: ClaudeOAuthCredentialsStore.pendingTokenCueOAuthKeychainCacheClearKey)
    ...
    private static let isolatedTestCredentialsURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("tokencue-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private struct Repository {
            ...
            clearInvalidCache: Bool = true) throws -> ClaudeOAuthCredentialRecord
        {
            try self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseTokenCueOAuthKeychainCache,
                   !ClaudeOAuthCredentialsStore.hasPendingTokenCueOAuthKeychainCacheClear(
                ...
                switch ClaudeOAuthCredentialsStore.loadTokenCueOAuthKeychainCache(
                    profileIdentifier: profileIdentifier)
                {
                ...
                case .temporarilyUnavailable:
                    ...
                    lastError = ClaudeOAuthCredentialsError.readFailed("TokenCue cache is temporarily unavailable.")
                case .missing:
                    break
                }
        ...
        private func prepareCachePolicy(environment: [String: String]) -> String {
            ...
            if !ClaudeOAuthCredentialsStore.shouldUseTokenCueOAuthKeychainCache {
                ClaudeOAuthCredentialsStore.markPendingTokenCueOAuthKeychainCacheClear(
                    profileIdentifier: profileIdentifier)
            }
            ...
            profileIdentifier: String) -> ClaudeOAuthCredentialRecord?
        {
            ...
            guard case let .found(entry) = ClaudeOAuthCredentialsStore.loadTokenCueOAuthKeychainCache(
                profileIdentifier: profileIdentifier),
        ...
        {
            guard owner == .tokencue else { return owner }
            ...
            environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool
        {
            self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseTokenCueOAuthKeychainCache {
                    if ClaudeOAuthCredentialsStore.hasPendingTokenCueOAuthKeychainCacheClear(
                        profileIdentifier: profileIdentifier)
                    {
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["TOKENCUE_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" {
                        ClaudeOAuthCredentialsStore.log.debug(
    ...
    /// - TokenCue-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to TokenCue's keychain cache
        ...
        profileIdentifier: String)
    {
        ...
        self.saveToCacheKeychain(
            ...
            historyOwnerIdentifier: historyOwnerIdentifier,
            profileIdentifier: profileIdentifier)
        self.log.debug("Saved refreshed credentials to TokenCue keychain cache")
    }
    ...
    /// Check if TokenCue has cached credentials (in memory or keychain cache)
    ...
    private static func saveToCacheKeychain(
        ...
        profileIdentifier: String)
    {
        guard self.shouldUseTokenCueOAuthKeychainCache else {
            self.markPendingTokenCueOAuthKeychainCacheClear(profileIdentifier: profileIdentifier)
            return
        }
        let entry = CacheEntry(
            ...
            historyOwnerIdentifier: historyOwnerIdentifier,
            profileIdentifier: profileIdentifier)
        self.currentPendingTokenCueOAuthKeychainCacheClearStore.withCacheTransaction(
            profileIdentifier: profileIdentifier,
    ...
    private static func clearCacheKeychain(profileIdentifier: String) -> Bool {
        if self.shouldUseTokenCueOAuthKeychainCache {
    ...
    private static func loadTokenCueOAuthKeychainCache(
        profileIdentifier: String) -> KeychainCacheStore.LoadResult<CacheEntry>
    {
        guard self.shouldUseTokenCueOAuthKeychainCache else { return .missing }
    ...
    private static func clearProfileCacheKeychain(profileIdentifier: String) -> Bool {
        guard self.shouldUseTokenCueOAuthKeychainCache else { return false }
    ...
    private static var shouldUseTokenCueOAuthKeychainCache: Bool {
        ClaudeOAuthKeychainPromptPreference.storedMode() != .never
    }

    private static func markPendingTokenCueOAuthKeychainCacheClear(profileIdentifier: String) {
        self.currentPendingTokenCueOAuthKeychainCacheClearStore.withCacheTransaction(
            profileIdentifier: profileIdentifier,
    ...
    private static func hasPendingTokenCueOAuthKeychainCacheClear(profileIdentifier: String) -> Bool {
        self.currentPendingTokenCueOAuthKeychainCacheClearStore.isPending(profileIdentifier: profileIdentifier)
    }

    #if DEBUG
    static var hasPendingTokenCueOAuthKeychainCacheClearForTesting: Bool {
        self.currentPendingTokenCueOAuthKeychainCacheClearStore.isPending
    }
    #endif

    private static var currentPendingTokenCueOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore {
        #if DEBUG
        ...
        #endif
        return self.pendingTokenCueOAuthKeychainCacheClearStore
    }
    ...
    static var keychainAccessAllowed: Bool {
        ...
        // replaces its ACL, so any permission granted to TokenCue is inherently temporary and causes recurring
        // macOS password dialogs. Production TokenCue therefore never reads the foreign item, with or without UI.
        return false
    }
    ...
    private static func credentialsProfileIdentifier(for credentialsURL: URL) -> String {
        ...
        let material = Data("tokencue:claude-oauth-cache-profile:v1\0\(path)".utf8)
    ...
    static func _resetCredentialsFileTrackingForTesting() {
        ...
        if self.taskPendingCacheClearStoreOverride != nil {
            self.currentPendingTokenCueOAuthKeychainCacheClearStore.withCacheTransaction { pending in
                pending = false
            }
        }
    }
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// TokenCue's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates TokenCue's keychain cache with the new credentials.
