...
private let miMoCookieImportOrder: BrowserCookieImportOrder =
    ProviderDefaults.metadata[.mimo]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources

public enum MiMoCookieImporter {
    private static let log = TokenCueLog.logger(LogCategories.provider(.mimo, scope: "cookie"))
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        return try self.importSessions(
            ...
            loadRecords: { browserSource, query, log in
                try Self.cookieClient.tokenCueRecords(
                    matching: query,
                    in: browserSource,
                    logger: log)
            })
            ...
            -> [BrowserCookieStoreRecords]) throws -> [SessionInfo]
    {
        try self.importSessions(
            ...
            loadStores: { try self.cookieClient.tokenCueStores(for: $0) })
    }
