...
enum FactoryLocalStorageImporter {
        ...
        logger: ((String) -> Void)? = nil) -> [TokenInfo]
    {
        ...
        if !chromeCandidates.isEmpty {
            log("Supported browser local storage candidates: \(chromeCandidates.count)")
        }

        let candidates = chromeCandidates
    ...
    static func hasSafariWorkOSRefreshToken() -> Bool {
        // Safari-only discovery is intentionally outside TokenCue's shared product contract.
        return false
    }
