...
public enum OllamaUsageError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            "Missing Ollama API key. Set apiKey in ~/.tokencue/config.json or OLLAMA_API_KEY."
        case .notLoggedIn:
        ...
        case .safariCookieAccessDenied:
            "Safari cookies need Full Disk Access for TokenCue (System Settings > Privacy & Security)."
        ...
        case let .browserCookieDecryptionDisabled(browserName):
            "\(browserName) cookie decryption is disabled in TokenCue; enable Keychain access and refresh."
        }
    }
}
...
private let ollamaCookieImportOrder: BrowserCookieImportOrder =
    ProviderDefaults.metadata[.ollama]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
...
public enum OllamaAPIUsageFetcher {
        ...
        now: Date = Date()) async throws -> OllamaAPIUsageSnapshot
    {
        ...
        request.setValue("TokenCue/1.0", forHTTPHeaderField: "User-Agent")
