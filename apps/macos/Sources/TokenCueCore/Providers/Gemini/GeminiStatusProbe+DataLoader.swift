...
extension GeminiStatusProbe {
        ...
        -> @Sendable (URLRequest) async throws -> (Data, URLResponse)
    {
        { request in
            do {
                ...
                TokenCueLog.logger(LogCategories.provider(.gemini, scope: "probe"))
    ...
    private static func curlDataLoader(for request: URLRequest) async throws -> (Data, URLResponse) {
        ...
        let tempDir = fileManager.temporaryDirectory
            .appendingPathComponent("tokencue-gemini-curl-\(UUID().uuidString)", isDirectory: true)
    ...
    private static let curlHTTPStatusMarker = "__TOKENCUE_HTTP_STATUS__:"
