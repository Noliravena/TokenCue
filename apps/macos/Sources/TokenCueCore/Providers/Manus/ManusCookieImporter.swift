...
public enum ManusCookieImporter {
    private static let log = TokenCueLog.logger(LogCategories.provider(.manus, scope: "cookie"))
    private static let cookieClient = BrowserCookieClient()
    private static let cookieDomains = ["manus.im", "www.manus.im"]
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.manus]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
    #if DEBUG
