...
public struct OneConsoleCookieHeaders: Sendable {
    ...
    public init?(cachedHeader raw: String?, cacheNamespace: String) {
        ...
        if let encodedAPI = valuesByName["__tokencue_\(cacheNamespace)_api"],
           let encodedDashboard = valuesByName["__tokencue_\(cacheNamespace)_dashboard"],
    ...
    public func cacheCookieHeader(namespace: String) -> String {
        [
            "__tokencue_\(namespace)_api=\(Self.encodeCachedHeader(self.apiCookieHeader))",
            "__tokencue_\(namespace)_dashboard=\(Self.encodeCachedHeader(self.dashboardCookieHeader))",
        ].joined(separator: "; ")
    }
