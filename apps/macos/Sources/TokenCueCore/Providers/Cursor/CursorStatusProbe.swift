...
private let cursorCookieImportOrder: BrowserCookieImportOrder =
    ProviderDefaults.metadata[.cursor]?.browserCookieOrder ?? TokenCueBrowserPolicy.supportedCookieSources
#endif
...
public enum CursorCookieImporter {
    ...
    /// profile that TokenCue cannot read. Ordinary background imports remain stricter.
        ...
        logger: ((String) -> Void)?) -> [SessionInfo]
    {
        ...
        do {
            ...
            let sources = try Self.cookieClient.tokenCueRecords(
                matching: query,
                in: browser,
                logger: log)
...
struct CursorAppAuthStore: CursorAppAuthSessionProviding {
        ...
        fileManager: FileManager = .default) -> String
    {
        ...
        #elseif os(Linux)
        let configHome = environment[TokenCueConfigStore.xdgConfigHomeEnvironmentKey]?
...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    static let safariFullDiskAccessHint =
        "If you use Safari, grant TokenCue Full Disk Access in System Settings ▸ Privacy & Security."

    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            #if os(macOS)
            "Not logged in to Cursor. Please log in via the TokenCue menu."
            #else
            "Not logged in to Cursor. Sign in to the Cursor app on this machine or paste a Cookie header copied "
                + "from cursor.com into ~/.config/tokencue/config.json (legacy: ~/.tokencue/config.json)."
            #endif
        ...
        case .noSessionCookie:
            ...
            "No Cursor session found. \(Self.safariFullDiskAccessHint) "
                ...
                + "You can also sign in to Cursor from the TokenCue menu (Add / switch account)."
            #else
...
public actor CursorSessionStore {
    ...
    private init() {
        ...
        let dir = appSupport.appendingPathComponent("TokenCue", isDirectory: true)
