...
enum TokenCueCLI {
    ...
    private static func bootstrapLogging(path: [String], values: ParsedValues) {
        TokenCueLog.bootstrapIfNeeded(self.loggingConfiguration(path: path, values: values))
    }

    static func loggingConfiguration(path: [String], values: ParsedValues) -> TokenCueLog.Configuration {
        ...
        let destination: TokenCueLog.Destination = path == ["diagnose"] ? .discard : .stderr
        return .init(destination: destination, level: level, json: isJSON)
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> TokenCueLog.Level {
        TokenCueLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
