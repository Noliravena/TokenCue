...
/// The CLI command encodes it directly while `tokencue serve` wraps it in the
...
struct DashboardSnapshotProducer: Sendable {
    ...
    let collectCost: @Sendable ([UsageProvider], TokenCueConfig) async -> [CostPayload]
    let now: @Sendable () -> Date
    var collectClaudeSwapAccounts: @Sendable (TokenCueConfig) async -> DashboardClaudeSwapCollection? = { _ in nil }
    var weeklyWorkDays: @Sendable () -> Int? = { nil }

    func collect(
        config: TokenCueConfig,
        refreshInterval: TimeInterval,
        tokenCueVersion: String?,
        identityMode: DashboardIdentityMode = .redacted) async throws -> DashboardSnapshotResult
    {
        let selection = TokenCueCLI.providerSelection(
            rawOverride: nil,
        ...
        let costPayloads = await self.collectCost(
            TokenCueCLI.costProviders(from: selection),
            config)
        ...
        let payload = DashboardSnapshotBuilder.makeSnapshot(
            ...
            tokenCueVersion: tokenCueVersion,
    ...
    static func live(context: DashboardSnapshotContext) -> Self {
        Self(
            ...
            collectCost: { providers, config in
                ...
                { provider, cursorCookieHeaderOverride in
                    do {
                        ...
                        return TokenCueCLI.makeCostPayload(provider: provider, snapshot: snapshot, error: nil)
                    } catch {
                        return TokenCueCLI.makeCostPayload(provider: provider, snapshot: nil, error: error)
                    }
                }
            },
            now: { Date() },
            collectClaudeSwapAccounts: { config in
                ...
                guard TokenCueCLI.dashboardClaudeSwapIsEligible(config: config) else { return nil }
            ...
            weeklyWorkDays: { TokenCueCLI.weeklyProgressWorkDaysFromDefaults() })
    }
}
...
extension TokenCueCLI {
    ...
    static func dashboardClaudeSwapIsEligible(config: TokenCueConfig) -> Bool {
    ...
    static func runDashboard(_ values: ParsedValues) async {
        ...
        let context = DashboardSnapshotContext(
            ...
            tokenCueVersion: Self.currentVersion())
        ...
        do {
            result = try await DashboardSnapshotProducer.live(context: context).collect(
                ...
                tokenCueVersion: context.tokenCueVersion,
                identityMode: identityMode)
        } catch {
    ...
    static func writeDashboardSnapshotAtomically(_ data: Data, toPath path: String) throws {
        ...
        let staged = directory.appendingPathComponent(
            ".\(url.lastPathComponent).tokencue-dashboard-\(UUID().uuidString)", isDirectory: false)
