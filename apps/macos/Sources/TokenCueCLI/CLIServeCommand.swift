...
struct ServeOptions: CommanderParsable {
    ...
    @Option(
        ...
        help: "Bearer token for /dashboard/v1/snapshot (prefer TOKENCUE_DASHBOARD_TOKEN)")
...
struct ServeRuntime {
    ...
    init(
        configStore: TokenCueConfigStore,
...
extension TokenCueCLI {
    ...
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("TokenCue server listening on http://\(bindHost):\(port)\n")
    ...
    static let dashboardTokenEnvironmentVariable = "TOKENCUE_DASHBOARD_TOKEN"
                ...
                makeResponse: {
                    await Self.serveDashboardSnapshot(
                        context: DashboardSnapshotContext(
                            ...
                            tokenCueVersion: runtime.healthVersion),
    ...
    static func loadServeConfigSnapshot(
        configStore: TokenCueConfigStore = TokenCueConfigStore()) throws -> CLIServeConfigSnapshot
    {
        let config = try configStore.load() ?? TokenCueConfig.makeDefault()
    ...
    static func serveConfigCacheToken(for config: TokenCueConfig) throws -> String {
    ...
    static func cachedServeResponse(
        ...
        cache: CLIServeResponseCache,
        refreshInterval: TimeInterval,
        requestTimeout: TimeInterval = TokenCueCLI.defaultServeRequestTimeout,
    ...
    private static func serveProviderSelection(
        rawProvider: String?,
        config: TokenCueConfig) throws -> ProviderSelection
    {
