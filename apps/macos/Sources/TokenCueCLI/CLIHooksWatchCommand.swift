...
extension TokenCueCLI {
    ...
    static func runHooksWatch(_ values: ParsedValues) async {
        ...
        guard hooks.enabled else {
            Self.exit(
                code: .failure,
                message: "Hooks are disabled. Run `tokencue hooks enable` first.",
                output: output,
                kind: .config)
        }
        guard !hooks.events.isEmpty else {
            Self.exit(
                code: .failure,
                message: "No hook rules configured. See `tokencue hooks list`.",
                output: output,
                kind: .config)
        }
    ...
    static func hooksWatchLanes(
        provider: UsageProvider,
        usage: UsageSnapshot,
        config: TokenCueConfig) -> [HookQuotaLaneObservation]
    {
    ...
    static func hooksWatchProviders(
        ...
        config: TokenCueConfig) -> Result<[UsageProvider], CLIArgumentError>
    {
