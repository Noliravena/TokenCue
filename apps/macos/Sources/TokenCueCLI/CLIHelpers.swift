...
extension TokenCueCLI {
    static func decodeProvider(from values: ParsedValues, config: TokenCueConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.tokencue.desktop",
            "com.tokencue.desktop.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> TokenCueConfig {
        let store = TokenCueConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return TokenCueConfig.makeDefault()
        } catch {
