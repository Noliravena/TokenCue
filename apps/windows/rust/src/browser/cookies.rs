...
impl CookieExtractor {
        ...
        if crate::wsl::is_wsl() {
            Err(CookieError::Dpapi(
                "DPAPI is not available in WSL. Chromium cookies cannot be automatically \
                 ...
                 instead. Run TokenCue natively on Windows for automatic cookie extraction."
                    .to_string(),
            ))
        } else {
            Err(CookieError::Dpapi(
    ...
    fn copy_to_temp(path: &Path) -> Result<std::path::PathBuf, CookieError> {
        ...
        let file_name = path
            ...
            .to_string_lossy()
            .to_string();
        let temp_path = temp_dir.join(format!("tokencue_{}_{}", uuid::Uuid::new_v4(), file_name));
...
mod tests {
    ...
    fn test_detect_abe_absent() {
        ...
        let path = dir.join("tokencue_test_local_state_no_abe.json");
        {
    ...
    fn test_detect_abe_present() {
        ...
        let dir = std::env::temp_dir();
        let path = dir.join("tokencue_test_local_state_abe.json");
        {
            let mut f = std::fs::File::create(&path).unwrap();
            write!(
                f,
