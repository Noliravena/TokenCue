...
const CODEBUFF_CREDENTIAL_TARGET: &str = "tokencue-codebuff";
...
impl CodebuffProvider {
    ...
    fn api_key_from_keyring() -> Option<String> {
        let entry =
            crate::core::credentials::keyring_entry(CODEBUFF_CREDENTIAL_TARGET, "api_key").ok()?;
    ...
    async fn fetch_usage_api(&self, ctx: &FetchContext) -> Result<UsageSnapshot, ProviderError> {
        ...
        let usage_resp = self
            ...
            .json(&json!({ "fingerprintId": "tokencue-usage" }))
            .send()
            .await?;
