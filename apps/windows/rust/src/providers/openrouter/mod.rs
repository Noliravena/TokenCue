...
/// OpenRouter API base URL — the bare `/api/v1` prefix.
///
/// Both endpoints append their path to this base: `/credits` and `/key`.
/// Do not bake `/auth` into the base (`.../api/v1/auth`), which would turn
/// the credits call into `/api/v1/auth/credits` -> 404.
...
const OPENROUTER_CREDENTIAL_TARGET: &str = "tokencue-openrouter";
...
impl OpenRouterProvider {
    ...
    fn get_api_token(api_key: Option<&str>) -> Result<String, ProviderError> {
        ...
        match crate::core::credentials::keyring_entry(
