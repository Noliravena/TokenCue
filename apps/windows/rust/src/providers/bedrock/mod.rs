...
impl BedrockProvider {
    ...
    fn profile_from_env() -> Option<String> {
        let mode = cleaned_env("TOKENCUE_BEDROCK_AUTH_MODE").map(|mode| mode.to_ascii_lowercase());
    ...
    fn monthly_budget() -> Option<f64> {
        cleaned_env("TOKENCUE_BEDROCK_BUDGET").and_then(|raw| {
            raw.parse::<f64>()
                .ok()
...
fn aws_cli_path() -> Result<String, ProviderError> {
    Ok(cleaned_env("TOKENCUE_AWS_CLI_PATH")
