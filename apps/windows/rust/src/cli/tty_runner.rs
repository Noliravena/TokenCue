...
impl TtyCommandRunner {
    ...
    fn run_pty_session(
        ...
        // Give the child time to initialize before sending scripted input. On
        // Windows, cmd.exe can issue an ANSI Device Status Report query while
        // ConPTY is starting and wait for the cursor-position response. If the
        // script is written before that handshake is answered, cmd.exe may
        // consume the script as terminal input without executing it.
        let startup_deadline = Instant::now()
            .checked_add(Duration::from_secs_f64(options.initial_delay_secs))
...
mod tests {
    ...
    fn test_run_sends_script_through_pty() {
        ...
        #[cfg(windows)]
        let result = runner.run("cmd", "echo TOKENCUE_PTY_OK\nexit", opts);
        #[cfg(not(windows))]
        let result = runner.run("sh", "echo TOKENCUE_PTY_OK\nexit", opts);

        let result = result.expect("pty command should run");
        assert!(result.text.contains("TOKENCUE_PTY_OK"), "{}", result.text);
    }
