//! `tokencue hooks` — list / enable / disable / test / watch external hook rules.
...
async fn run_watch(args: HooksWatchArgs) -> anyhow::Result<()> {
    ...
    if !hooks.enabled {
        anyhow::bail!("Hooks are disabled. Run `tokencue hooks enable` first.");
    }
    if hooks.events.is_empty() {
        anyhow::bail!("No hook rules configured. See `tokencue hooks list`.");
    }
...
mod tests {
    ...
    fn sample_quota_low_has_remaining() {
        ...
        assert!(e.environment_variables().contains_key("TOKENCUE_PROVIDER"));
    }
