...
const DASHBOARD_TOKEN_ENV: &str = "TOKENCUE_DASHBOARD_TOKEN";
...
pub struct ServeArgs {
    ...
    /// Bearer token for /usage and /cost (prefer TOKENCUE_DASHBOARD_TOKEN)
    #[arg(long = "dashboard-token", env = "TOKENCUE_DASHBOARD_TOKEN")]
...
pub async fn run(args: ServeArgs) -> anyhow::Result<()> {
    ...
    eprintln!(
        "TokenCue server listening on http://{}:{}",
