...
async fn list_accounts(provider_name: &str) -> anyhow::Result<()> {
    ...
    if data.accounts.is_empty() {
        ...
        println!(
            "Use 'tokencue account add {}' to add one.",
            provider.cli_name()
        );
        return Ok(());
    }
...
fn parse_provider(name: &str) -> anyhow::Result<ProviderId> {
    ProviderId::from_cli_name(name)
        .ok_or_else(|| anyhow::anyhow!("Unknown provider: '{}'. Use 'tokencue usage --provider all' to see available providers.", name))
}
...
fn find_account<'a>(
    ...
    anyhow::bail!(
        "Account '{}' not found. Use 'tokencue account list' to see available accounts.",
        account_ref
    )
}
