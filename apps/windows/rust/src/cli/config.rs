...
fn parse_provider(raw: &str) -> anyhow::Result<ProviderId> {
    ProviderId::from_cli_name(raw).ok_or_else(|| {
        anyhow::anyhow!(
            "Unknown provider '{}'. Run `tokencue config providers` to list providers.",
            raw
        )
    })
}
...
async fn show_paths() -> anyhow::Result<()> {
    ...
    if let Some(config_dir) = dirs::config_dir() {
        let tokencue_dir = config_dir.join("TokenCue");
        println!();
        println!("Config directory: {}", tokencue_dir.display());
    }
