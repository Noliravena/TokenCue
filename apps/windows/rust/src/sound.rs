//! Notification sound playback for TokenCue.
//!
//! Handles per-event custom WAV files, built-in TokenCue sounds, and Windows system sounds.
...
pub fn play_alert(event: NotificationSoundEvent, settings: &Settings) -> Result<(), SoundError> {
    ...
    match settings.notification_sound_theme {
        ...
        NotificationSoundTheme::TokenCue => play_built_in_sound(event),
    }
}
...
fn enqueue_playback(request: PlaybackRequest) -> Result<(), SoundError> {
    let sender = PLAYBACK_QUEUE.get_or_init(|| {
        ...
        std::thread::Builder::new()
            .name("tokencue-notification-sound".to_string())
