...
//! `{cache}/TokenCue/cost-usage/`, skips unchanged files by mtime+size, resumes
...
mod tests {
    ...
    fn parses_current_codex_payload_token_count_events() {
        let path = std::env::temp_dir().join(format!(
            "tokencue-current-codex-token-count-{}.jsonl",
            std::process::id()
        ));
    ...
    fn daily_history_dedups_across_files_and_buckets_by_local_day() {
        ...
        let file_a = dir.join(format!(
            "tokencue-claude-daily-a-{}.jsonl",
            std::process::id()
        ));
        let file_b = dir.join(format!(
            "tokencue-claude-daily-b-{}.jsonl",
            std::process::id()
        ));
    ...
    fn claude_scan_counts_final_incomplete_jsonl_line() {
        let path =
            std::env::temp_dir().join(format!("tokencue-claude-tail-{}.jsonl", std::process::id()));
