...
fn test_settings_default() {
    ...
    assert_eq!(settings.critical_usage_threshold, 95.0);
...
fn notification_sound_paths_round_trip_and_default_for_existing_settings() {
    let settings = Settings {
        notification_sound_theme: NotificationSoundTheme::TokenCue,
...
fn test_settings_toggle_provider() {
    ...
    // Fresh installs start with no providers until onboarding consent.
    assert!(!settings.is_provider_enabled(ProviderId::Claude));
...
fn test_settings_get_enabled_provider_ids() {
    let settings = Settings::default();
    assert!(settings.get_enabled_provider_ids().is_empty());
}
