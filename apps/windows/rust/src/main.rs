...
fn launch_log_path() -> PathBuf {
    std::env::temp_dir().join(format!("tokencue_launch_{}.log", std::process::id()))
}
...
fn dispatch_command(rt: &Runtime, command: Option<Commands>) -> i32 {
    match command {
        ...
        Some(Commands::Plugins(args)) => run_unexpected(rt, cli::plugins::run(args)),
...
fn missing_subcommand() -> i32 {
    ...
    eprintln!(
        "tokencue is CLI-only. Run a subcommand (e.g. `tokencue usage -p claude`) \
         or launch the Tauri desktop shell via `apps/desktop-tauri`.\n\
         Use `tokencue --help` for the full list of subcommands."
    );
...
mod tests {
    ...
    fn launch_log_path_is_process_scoped() {
        ...
        assert!(file_name.starts_with("tokencue_launch_"));
