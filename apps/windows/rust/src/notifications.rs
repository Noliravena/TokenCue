//! System notifications for TokenCue
//!
...
impl NotificationManager {
    ...
    fn show_toast(&self, title: &str, body: &str) {
        ...
        // CreateToastNotifier("TokenCue.Desktop") finds a valid registration rather than
...
'@
    ...
    $notifier = [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("TokenCue.Desktop")
    if ($null -eq $notifier) {{ throw "CreateToastNotifier returned null" }}
    $notifier.Show($toast)
}} catch {{
    [System.Console]::Error.WriteLine("TokenCue toast failed: $_")
    exit 1
}}"#,
...
/// Register the TokenCue App User Model ID (AUMID) in the Windows registry so that
/// `CreateToastNotifier("TokenCue.Desktop")` resolves to a valid notifier instead of returning
...
fn ensure_aumid_registered() {
    ...
    let result = hkcu
        .create_subkey(r"SOFTWARE\Classes\AppUserModelId\TokenCue.Desktop")
        .and_then(|(key, _)| key.set_value("DisplayName", &"TokenCue"));

    match result {
        Ok(()) => {
            tracing::debug!("TokenCue.Desktop AUMID registered for Windows toast notifications")
        }
        Err(e) => tracing::warn!("Failed to register TokenCue.Desktop AUMID: {}", e),
    }
}
