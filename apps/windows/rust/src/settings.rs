//! Settings management for TokenCue
//!
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            // A fresh TokenCue install must not touch browser or CLI credentials
            // until the user explicitly selects providers in onboarding.
...
impl Settings {
    ...
    pub fn settings_path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("TokenCue").join("settings.json"))
    }
    ...
    fn promote_tray_default_marker_path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("TokenCue").join(".tray-pin-default-v1"))
    }
