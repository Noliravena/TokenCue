...
impl TokenAccountSupport {
    ...
    pub fn for_provider(provider: ProviderId) -> Option<Self> {
        match provider {
            ...
            ProviderId::Moonshot => Some(TokenAccountSupport {
                title: "API keys",
                subtitle: "Store multiple Moonshot / Kimi Open Platform API keys.",
            ...
            ProviderId::Synthetic => Some(TokenAccountSupport {
                title: "API keys",
                subtitle: "Store multiple Synthetic API keys.",
                placeholder: "Synthetic API key",
                injection: TokenInjection::Environment {
                    key: "SYNTHETIC_API_KEY".to_string(),
                },
                requires_manual_cookie_source: false,
                cookie_name: None,
            }),
            ProviderId::ClawRouter => Some(TokenAccountSupport {
                title: "API keys",
                subtitle: "Store multiple ClawRouter API keys.",
                placeholder: "ClawRouter API key",
                injection: TokenInjection::Environment {
                    key: "CLAWROUTER_API_KEY".to_string(),
                },
                requires_manual_cookie_source: false,
                cookie_name: None,
            }),
    ...
    pub fn env_override(provider: ProviderId, token: &str) -> Option<HashMap<String, String>> {
        ...
        match &support.injection {
            ...
            TokenInjection::CookieHeader => {
                ...
                {
                    let mut map = HashMap::new();
                    map.insert("TOKENCUE_CLAUDE_OAUTH_TOKEN".to_string(), normalized);
                    return Some(map);
                }
                None
            }
