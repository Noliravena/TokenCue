...
/// honors these options and persists cache under `{cache}/TokenCue/cost-usage/`.
...
impl JsonlScanner {
    ...
    /// Default on-disk cache root: `%LOCALAPPDATA%\TokenCue` (via `dirs::cache_dir`).
    pub fn default_cache_root() -> Option<PathBuf> {
        dirs::cache_dir().map(|d| d.join("TokenCue"))
    }
