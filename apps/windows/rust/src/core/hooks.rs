...
impl HookEvent {
    ...
    /// `TOKENCUE_*` environment variables for the hook process.
    pub fn environment_variables(&self) -> HashMap<String, String> {
        let mut env = HashMap::new();
        env.insert("TOKENCUE_EVENT".into(), self.event.as_str().into());
        env.insert("TOKENCUE_PROVIDER".into(), self.provider.clone());
        env.insert("TOKENCUE_TIMESTAMP".into(), self.timestamp.clone());
        if let Some(account) = &self.account {
            env.insert("TOKENCUE_ACCOUNT".into(), account.clone());
        }
        if let Some(window) = &self.window {
            env.insert("TOKENCUE_WINDOW".into(), window.clone());
        }
        if let Some(usage) = self.usage_percent {
            env.insert("TOKENCUE_USAGE_PERCENT".into(), format_number(usage));
        }
        if let Some(remaining) = self.remaining_percent {
            env.insert(
                "TOKENCUE_REMAINING_PERCENT".into(),
                format_number(remaining),
            );
        }
        if let Some(status) = &self.status {
            env.insert("TOKENCUE_STATUS".into(), status.clone());
        }
        env
    }
...
impl HooksConfig {
    ...
    pub fn path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("TokenCue").join("hooks.json"))
    }
...
mod tests {
    ...
    fn payload_env_includes_event_and_remaining() {
        ...
        assert_eq!(
            env.get("TOKENCUE_EVENT").map(String::as_str),
            Some("quota_low")
        );
        assert_eq!(
            env.get("TOKENCUE_PROVIDER").map(String::as_str),
            Some("claude")
        );
        assert_eq!(
            env.get("TOKENCUE_WINDOW").map(String::as_str),
            Some("session")
        );
        assert_eq!(
            env.get("TOKENCUE_ACCOUNT").map(String::as_str),
            Some("user@example.com")
        );
        assert_eq!(
            env.get("TOKENCUE_USAGE_PERCENT").map(String::as_str),
            Some("0.8")
        );
        assert_eq!(
            env.get("TOKENCUE_REMAINING_PERCENT").map(String::as_str),
            Some("20")
        );
