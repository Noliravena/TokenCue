...
// The handoff flyout is content-sized. Keep only a small bootstrap height so
// WebView2 can complete its first layout pass, then let the measured surface
// determine the real window height. The previous 200/420px floors left a
// large canvas-colored void below short provider lists.
const TRAY_CONTENT_BOOTSTRAP_HEIGHT = 120;
...
export function useTrayPanelLayout({
  ...
  useEffect(() => {
    const surface = document.querySelector<HTMLElement>(
      ".tokencue-tray, .menu-surface--tray",
    );
  ...
  useEffect(() => {
    ...
    const minHeight = TRAY_CONTENT_BOOTSTRAP_HEIGHT;
    ...
    const resize = async () => {
      ...
      const providerList = surface.querySelector<HTMLElement>(
        ".tokencue-tray__providers",
      );
      ...
      const previous = {
        ...
        providerListMaxHeight: providerList?.style.maxHeight,
        providerListOverflow: providerList?.style.overflow,
      };
      ...
      // The handoff list normally caps itself against 100vh so a genuinely
      // tall catalog scrolls. During the measurement pass the window is only
      // 120px high, so that cap would clip the row before scrollHeight can see
      // its progress bar. Temporarily lift it, measure, then restore it.
      if (providerList) {
        providerList.style.maxHeight = "none";
        providerList.style.overflow = "visible";
      }
      ...
      try {
        ...
        const footer = surface.querySelector<HTMLElement>(
          ".tokencue-tray__footer, .menu-surface__footer",
        );
        ...
        console.warn("TokenCue tray panel resize failed", error);
        void revealPanel();
      } finally {
        if (!committedHeight) {
        ...
        if (providerList) {
          providerList.style.maxHeight = previous.providerListMaxHeight ?? "";
          providerList.style.overflow = previous.providerListOverflow ?? "";
        }
        window.setTimeout(() => {
