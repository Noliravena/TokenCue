...
mod windows {
    ...
    fn find_own_subkey(
        ...
        // rewrites where Windows still has a single TokenCue notify entry).
        if name_matches == 1 {
            return Ok(by_name);
        }
        Ok(None)
...
mod tests {
    ...
    fn matches_same_path_case_insensitive() {
        assert!(matches_current_exe(
            r"C:\APPS\TokenCue.exe",
            Path::new(r"c:\apps\tokencue.exe")
        ));
    }
    ...
    fn rejects_different_exe_same_dir() {
        assert!(!matches_current_exe(
            r"C:\apps\Other.exe",
            Path::new(r"C:\apps\TokenCue.exe")
        ));
    }
    ...
    fn matches_extended_length_prefix_from_current_exe() {
        assert!(matches_current_exe(
            r"C:\Users\mac\AppData\Local\Programs\TokenCue\tokencue-desktop.exe",
            Path::new(r"\\?\C:\Users\mac\AppData\Local\Programs\TokenCue\tokencue-desktop.exe")
        ));
    }
    ...
    fn matches_exe_file_name_across_install_dirs() {
        assert!(matches_exe_file_name(
            r"C:\Old\TokenCue\tokencue-desktop.exe",
            Path::new(r"C:\Users\mac\AppData\Local\Programs\TokenCue\tokencue-desktop.exe")
        ));
        assert!(!matches_exe_file_name(
            r"C:\Old\TokenCue\tokencue.exe",
            Path::new(r"C:\Users\mac\AppData\Local\Programs\TokenCue\tokencue-desktop.exe")
        ));
    }
    ...
    fn normalize_strips_unc_extended_prefix() {
        assert_eq!(
            normalize_exe_path(r"\\?\UNC\server\share\tokencue-desktop.exe"),
            r"\\server\share\tokencue-desktop.exe"
        );
    }
