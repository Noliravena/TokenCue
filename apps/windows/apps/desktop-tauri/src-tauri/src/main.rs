...
fn main() {
    tokencue::logging::init(false, false).expect("failed to initialize logging");

    let proof_config = proof_harness::ProofConfig::from_env();
    let is_proof_mode = proof_config.is_some();
    let force_start_visible = std::env::var_os("TOKENCUE_START_VISIBLE").is_some();
    ...
    tauri::Builder::default()
        ...
        .expect("failed to run TokenCue desktop shell");
}
