...
use tokencue::core::OpenAIDashboardCacheStore;
use tokencue::cost_scanner::{CostScanner, CostSummary, get_daily_cost_history};
use tokencue::locale::{self, LocaleKey};
...
pub(crate) async fn refresh_provider_local_usage_cache(provider_ids: Vec<String>) {
    ...
    for (provider_id, mut summary, unknown_models) in scans {
        ...
        if let Some(pricing_provider) = pricing_provider
            && tokencue::core::refresh_unknown_models_if_needed(pricing_provider, &unknown_models)
                .await
        {
...
fn localized_estimate_note(provider_id: &str, lang: tokencue::settings::Language) -> String {
    match provider_id {
