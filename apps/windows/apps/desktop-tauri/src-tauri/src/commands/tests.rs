...
use tokencue::host::session::launch_block_reason;
use tokencue::settings::{ApiKeys, Language, ManualCookies, Settings};
...
fn external_url_validation_allows_only_http_urls() {
    assert_eq!(
        validate_external_url(" https://github.com/Noliravena/TokenCue ").unwrap(),
...
fn credential_status_labels_do_not_include_error_details() {
    assert_eq!(
        super::credential_file_status_label(tokencue::secure_file::SecureFileStatus::Missing),
        "missing"
    );
    assert_eq!(
        super::credential_file_status_label(tokencue::secure_file::SecureFileStatus::Plaintext),
        "plaintext"
    );
    assert_eq!(
        super::credential_file_status_label(tokencue::secure_file::SecureFileStatus::Protected(
            "windows-dpapi-user".to_string(),
        )),
        "protected:windows-dpapi-user"
    );
    assert_eq!(
        super::credential_file_status_label(tokencue::secure_file::SecureFileStatus::Unreadable(
...
fn apply_provider_order_dedupes_and_appends_unknown_canonical() {
    ...
    let order =
        tokencue::settings::normalize_provider_order(&["codex".to_string(), "claude".to_string()]);
    ...
    let canonical = tokencue::core::ProviderId::all()
        .iter()
...
fn apply_provider_order_ignores_unknown_ids() {
    let order = tokencue::settings::normalize_provider_order(&[
...
fn provider_summaries_reflect_settings_order() {
    ...
    let canonical_len = tokencue::core::ProviderId::all()
        .iter()
...
fn provider_catalog_preserves_partial_config_order() {
    let settings = Settings {
        provider_order: tokencue::settings::normalize_provider_order(&[
            "gemini".to_string(),
            "claude".to_string(),
            "codex".to_string(),
        ]),
...
fn provider_cache_upsert_replaces_existing_provider() {
    ...
    let result = ProviderFetchResult {
        usage: tokencue::core::UsageSnapshot::new(tokencue::core::RateWindow::new(10.0)),
        cost: None,
...
fn claude_transient_auth_failure_preserves_first_last_good_snapshot() {
    ...
    let result = ProviderFetchResult {
        usage: tokencue::core::UsageSnapshot::new(tokencue::core::RateWindow::new(42.0)),
        cost: None,
        wayfinder_usage: None,
...
fn claude_cli_parse_failure_keeps_last_good_every_time() {
    let metadata = instantiate_provider(ProviderId::Claude).metadata().clone();
    let result = ProviderFetchResult {
        usage: tokencue::core::UsageSnapshot::new(tokencue::core::RateWindow::new(17.0)),
        cost: None,
        wayfinder_usage: None,
        source_label: "CLI".to_string(),
    };
...
fn claude_error_message_explains_missing_sign_in() {
    ...
    assert_eq!(
        message,
        "Claude sign-in was not found. Run `claude` once to authenticate, then refresh Claude in TokenCue."
    );
}
...
fn japanese_provider_snapshot_localizes_weekly_label() {
    let metadata = instantiate_provider(ProviderId::Claude).metadata().clone();
    let usage = tokencue::core::UsageSnapshot::new(tokencue::core::RateWindow::new(10.0))
        .with_secondary(tokencue::core::RateWindow::new(20.0));
    let result = ProviderFetchResult {
        usage,
        cost: None,
        wayfinder_usage: None,
...
fn japanese_provider_snapshot_localizes_pace_reserve_description() {
    ...
    let secondary = tokencue::core::RateWindow::with_details(
        40.0,
        Some(7 * 24 * 60),
...
fn open_path_rejects_missing_path() {
    let missing = std::env::temp_dir()
        .join(format!("tokencue-phase6d-missing-{}", std::process::id()))
        .join("does-not-exist");
...
fn external_url_validator_accepts_http_and_https() {
    assert_eq!(
        super::validate_external_url(" https://github.com/Noliravena/TokenCue "),
        Ok("https://github.com/Noliravena/TokenCue")
    );
    assert_eq!(
