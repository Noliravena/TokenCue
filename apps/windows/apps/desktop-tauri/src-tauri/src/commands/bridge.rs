...
impl RateWindowSnapshot {
    ...
    fn with_pace_reserve(mut self, pace: &tokencue::core::UsagePace) -> Self {
...
pub struct ProviderUsageSnapshot {
    ...
    pub wayfinder_usage: Option<tokencue::core::WayfinderUsageSnapshot>,
...
pub(crate) fn pace_stage_str(stage: tokencue::core::PaceStage) -> &'static str {
...
impl ProviderUsageSnapshot {
    pub(super) fn from_fetch_result(
        ...
        let primary_pace = tokencue::core::UsagePace::weekly(&usage.primary, None, 10080);
        ...
        let secondary_pace = usage
            .secondary
            .as_ref()
            .and_then(|sw| tokencue::core::UsagePace::weekly(sw, None, 10080));
...
fn session_equivalent_forecast_for(
    ...
    tokencue::core::record_provider_windows(provider_id, session, Some(weekly), now);
    let work_days = Settings::load().weekly_progress_work_days;
    let forecast =
        tokencue::core::forecast_for_provider(provider_id, session, weekly, now, work_days)?;
...
pub(crate) fn compact_tray_status_label(
    ...
    lang: tokencue::settings::Language,
) -> String {
...
fn normalize_reset_description(desc: &str, lang: tokencue::settings::Language) -> String {
...
pub(crate) fn friendly_provider_error(id: ProviderId, error: &str) -> String {
    ...
    if lower.contains("claude oauth credentials not found") {
        return "Claude sign-in was not found. Run `claude` once to authenticate, then refresh Claude in TokenCue.".to_string();
    }

    if lower.contains("oauth token expired") || lower.contains("token invalid or expired") {
        return "Claude sign-in expired. Run `claude` to refresh your Claude Code login, then refresh Claude in TokenCue.".to_string();
    }

    if trimmed == "Authentication required" {
        return "Claude needs sign-in before TokenCue can read usage. Run `claude` once, or add Claude cookies in Provider settings.".to_string();
    }
...
pub struct SettingsSnapshot {
    ...
    sound_enabled: bool,
    notification_sound_theme: tokencue::settings::NotificationSoundTheme,
    notification_sound_paths: tokencue::settings::NotificationSoundPaths,
    high_usage_threshold: f64,
    critical_usage_threshold: f64,
    provider_usage_thresholds:
        std::collections::HashMap<String, tokencue::settings::UsageThresholdOverride>,
...
mod tests {
    ...
    fn japanese_tray_status_label_has_no_english_reset_text() {
        use tokencue::settings::Language;
