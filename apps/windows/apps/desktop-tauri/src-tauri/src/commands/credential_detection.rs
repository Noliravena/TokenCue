...
fn gemini_cli_credentials_path() -> Option<std::path::PathBuf> {
    tokencue::host::session::gemini_cli_credentials_path()
}

fn vertexai_credentials_path_raw() -> Option<std::path::PathBuf> {
    tokencue::host::session::vertexai_credentials_path()
}

fn jetbrains_detected_ide_paths() -> Vec<std::path::PathBuf> {
    tokencue::host::session::jetbrains_detected_ide_paths()
}
...
pub fn get_kiro_status() -> Result<KiroStatus, String> {
    if let Some(path) = tokencue::providers::kiro::find_kiro_cli() {
        Ok(KiroStatus {
            available: true,
