...
pub(crate) fn provider_cookie_domain(id: ProviderId, settings: &Settings) -> Option<&'static str> {
    ...
    if id == ProviderId::Alibaba {
        return Some(
            tokencue::providers::AlibabaProvider::cookie_domain_for_region(Some(
                settings.api_region(id),
            )),
        );
    }
    ...
    let mut snapshot =
            ...
            .await
        {
            ...
            Ok(Err(e)) => ProviderUsageSnapshot::from_error(
                id,
                &metadata,
                tokencue::logging::safe_error_message(e),
            ),
...
fn notify_usage_thresholds(
    ...
    let cli_map = tokencue::core::cli_name_map();
    if let Ok(mut guard) = state.lock() {
...
fn dispatch_quota_hooks(
    ...
    tokencue::core::emit_quota_threshold_hooks(
        true,
        provider.cli_name(),
        window,
        used_percent,
...
fn notify_predictive_pace(
    manager: &mut tokencue::notifications::NotificationManager,
    provider: ProviderId,
    snapshot: &ProviderUsageSnapshot,
    token_accounts: &HashMap<ProviderId, ProviderAccountData>,
    settings: &Settings,
    ...
    for (warning_window, window, default_window_minutes) in [
        (
            tokencue::notifications::PredictiveWarningWindow::Session,
            Some(&snapshot.primary),
            300,
        ),
        (
            tokencue::notifications::PredictiveWarningWindow::Weekly,
        ...
        let Some(pace) =
            tokencue::core::UsagePace::weekly(&rate_window, observed_at, default_window_minutes)
        else {
            continue;
        };
...
mod predictive_warning_tests {
    ...
    fn empty_snapshot() -> ProviderUsageSnapshot {
        let metadata = tokencue::core::instantiate_provider(ProviderId::Claude)
            .metadata()
            .clone();
