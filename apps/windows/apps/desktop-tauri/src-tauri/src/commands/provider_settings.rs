...
pub fn reorder_providers(
    ...
    settings.provider_order = tokencue::settings::normalize_provider_order(&ids);
...
fn cookie_source_provider(provider_id: &str) -> Option<tokencue::core::ProviderId> {
...
fn region_provider(provider_id: &str) -> Option<tokencue::core::ProviderId> {
    use tokencue::core::ProviderId;
    Some(match provider_id {
        "alibaba" => ProviderId::Alibaba,
...
pub(crate) fn provider_region_lookup(settings: &Settings, provider_id: &str) -> Option<String> {
    region_provider(provider_id).map(|id| {
        if id == tokencue::core::ProviderId::MiniMax {
            tokencue::providers::MiniMaxProvider::region_from_settings(Some(
...
fn workspace_provider(provider_id: &str) -> Option<tokencue::core::ProviderId> {
    use tokencue::core::ProviderId;
    Some(match provider_id {
        "openai" | "openaiapi" => ProviderId::OpenAIApi,
...
pub fn set_provider_workspace_id(provider_id: String, workspace_id: String) -> Result<(), String> {
    ...
    let workspace_id = tokencue::settings::validate_provider_workspace_value(id, &workspace_id)?;
    let mut settings = Settings::load();
...
fn litellm_workspace_change_allowed(
    ...
    if id != tokencue::core::ProviderId::LiteLLM || next_workspace_id.trim().is_empty() {
        return Ok(());
    }
...
fn gateway_provider(provider_id: &str) -> Option<tokencue::core::ProviderId> {
    (provider_id == "wayfinder").then_some(tokencue::core::ProviderId::Wayfinder)
}
...
pub fn set_provider_gateway_url(provider_id: String, gateway_url: String) -> Result<(), String> {
    ...
    tokencue::providers::wayfinder::parse_gateway_url(gateway_url)
...
pub fn region_options_for(provider_id: &str) -> Vec<RegionOption> {
    match provider_id {
        "alibaba" => tokencue::providers::AlibabaRegion::ALL
            .iter()
        ...
        "minimax" => vec![
            RegionOption {
                value: "global".to_string(),
                label: tokencue::providers::MiniMaxRegion::Global
                    .display_name()
                    .to_string(),
            },
            RegionOption {
                value: "cn".to_string(),
                label: tokencue::providers::MiniMaxRegion::ChinaMainland
                    .display_name()
                    .to_string(),
            },
        ],
        "alibabatokenplan" => tokencue::providers::AlibabaTokenPlanRegion::ALL
            .iter()
            .copied()
            .map(|region| RegionOption {
