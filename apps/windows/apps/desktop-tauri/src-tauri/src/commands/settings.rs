...
pub struct SettingsUpdate {
    ...
    pub notification_sound_theme: Option<tokencue::settings::NotificationSoundTheme>,
    pub notification_sound_paths: Option<tokencue::settings::NotificationSoundPaths>,
    pub high_usage_threshold: Option<f64>,
    pub critical_usage_threshold: Option<f64>,
    pub provider_usage_thresholds:
        Option<std::collections::HashMap<String, tokencue::settings::UsageThresholdOverride>>,
...
impl SettingsUpdate {
    ...
    fn apply_display_settings(self, settings: &mut Settings) -> Self {
        ...
        if let Some(v) = self.window_scale_percent {
            settings.window_scale_percent = tokencue::settings::clamp_window_scale_percent(v);
        }
        if let Some(v) = self.tray_scale_percent {
            settings.tray_scale_percent = tokencue::settings::clamp_tray_scale_percent(v);
        }
    ...
    fn apply_notification_settings(self, settings: &mut Settings) -> Result<Self, String> {
        ...
        if let Some(v) = self.notification_sound_paths.clone() {
            tokencue::sound::validate_custom_sound_path_updates(
        ...
        if let Some(values) = self.provider_usage_thresholds.clone() {
            settings.provider_usage_thresholds =
                tokencue::settings::normalize_usage_threshold_overrides(values);
        }
    ...
    fn apply_advanced_settings(self, settings: &mut Settings) -> Self {
        ...
        if let Some(v) = self.agent_session_ssh_hosts.clone() {
            settings.agent_session_ssh_hosts =
                tokencue::agent_sessions::RemoteSessionFetcher::sanitized_hosts(&v);
        }
        ...
        if let Some(v) = self.alibaba_token_plan_region.as_deref() {
            let region = tokencue::providers::AlibabaTokenPlanRegion::from_settings_value(Some(v));
            settings.set_api_region(
                tokencue::core::ProviderId::AlibabaTokenPlan,
                region.as_str(),
            );
        }
...
mod tests {
    ...
    fn apply_notification_settings_updates_sound() {
        ...
        SettingsUpdate {
            notification_sound_theme: Some(tokencue::settings::NotificationSoundTheme::TokenCue),
            ..Default::default()
        }
        ...
        assert_eq!(
            ...
            tokencue::settings::NotificationSoundTheme::TokenCue
        );
    }
    ...
    fn apply_notification_settings_rejects_invalid_custom_sound() {
        ...
        let result = SettingsUpdate {
            notification_sound_paths: Some(tokencue::settings::NotificationSoundPaths {
        ...
        assert_eq!(
            settings.notification_sound_paths,
            tokencue::settings::NotificationSoundPaths::default()
        );
    }
}
