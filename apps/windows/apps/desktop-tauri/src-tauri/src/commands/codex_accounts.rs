...
pub async fn codex_account_fetch(
    app: tauri::AppHandle,
    id: String,
) -> Result<tokencue::codex_accounts::AccountUsageSnapshot, String> {
    let accounts = load_codex_accounts()?;
    let target = accounts
        .iter()
        .find(|account| account.id.to_string() == id)
...
-> Result<HashMap<Uuid, tokencue::codex_accounts::AccountUsageSnapshot>, String> {
...
pub struct CodexAccountsStateBridge {
    ...
    pub snapshots: HashMap<Uuid, tokencue::codex_accounts::AccountUsageSnapshot>,
}
...
mod tests {
    ...
    fn sample_account() -> CodexAccount {
        CodexAccount::new(
            ...
            tokencue::codex_accounts::CodexAccountSource::ManagedByApp,
            tokencue::codex_accounts::utc_now(),
            tokencue::codex_accounts::utc_now(),
            Some(tokencue::codex_accounts::utc_now()),
        )
    }
