...
fn schedule_tray_promotion_retries(app_handle: AppHandle) {
    if !tokencue::settings::Settings::load().promote_tray_icon {
        return;
    }
...
pub(crate) fn rebuild_tray_menu(app: &AppHandle) {
    ...
    if let Ok(menu) = build_native_tray_menu(app, &catalog, &status_labels)
        && let Some(tray) = app.tray_by_id("tokencue-main")
    {
...
pub fn update_tray_icon_and_tooltip(
    ...
    let Some(tray) = app.tray_by_id("tokencue-main") else {
        return;
    };
...
fn build_tooltip(
    ...
    use tokencue::locale::{LocaleKey, get_text};

    if snapshots.is_empty() {
        return "TokenCue".to_string();
    }
...
mod tests {
    ...
    fn status_labels_per_provider_mode_lists_each_healthy_provider() {
        ...
        let labels = status_labels_for_settings(
            &settings,
            &snapshots,
            tokencue::settings::Language::English,
        );
    ...
    fn tooltip_uses_compact_status_labels() {
        ...
        let tooltip = build_tooltip(&[claude, codex], tokencue::settings::Language::English);
    ...
    fn tooltip_truncates_long_provider_lines() {
        ...
        let tooltip = build_tooltip(&[claude], tokencue::settings::Language::English);
    ...
    fn japanese_tooltip_localizes_error_status() {
        ...
        let tooltip = build_tooltip(&[claude], tokencue::settings::Language::Japanese);
    ...
    fn tray_labels_relocalize_on_language_change_without_refetch() {
        ...
        let english_tooltip =
            build_tooltip(&[claude.clone()], tokencue::settings::Language::English);
        let japanese_tooltip =
            build_tooltip(&[claude.clone()], tokencue::settings::Language::Japanese);
        ...
        let (_, english_label) =
            provider_status_label(&claude, tokencue::settings::Language::English);
        let (_, japanese_label) =
            provider_status_label(&claude, tokencue::settings::Language::Japanese);
