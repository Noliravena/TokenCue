...
fn geometry_path() -> Option<PathBuf> {
    // Reuse the same TokenCue config directory as Settings, so remembered
    // geometry lives alongside `settings.json` on every platform.
    tokencue::settings::Settings::settings_path()
...
pub fn save_entry(key: &str, geometry: StoredGeometry) {
    ...
    if let Err(err) = save_file(&file) {
        tracing::warn!(target: "tokencue::geometry", %err, "failed to persist geometry");
    }
}
...
pub fn save_size(key: &str, size: StoredSize) {
    ...
    if let Err(err) = save_file(&file) {
        tracing::warn!(target: "tokencue::geometry", %err, "failed to persist size");
    }
}
