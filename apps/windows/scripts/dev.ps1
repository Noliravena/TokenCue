...
<#
.SYNOPSIS
    Build and run the TokenCue Tauri desktop shell for Windows.
...
$DesktopBinaryName = "tokencue-desktop.exe"
...
if (-not $SkipBuild) {
    ...
    try {
        if ($Release) {
            Write-Host "Building TokenCue Desktop (release, no bundle)..." -ForegroundColor Cyan
            & $pnpmCommand.Source run tauri:build
        } else {
            Write-Host "Building TokenCue Desktop (debug, no bundle)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Starting TokenCue Desktop..." -ForegroundColor Green
& $binary
