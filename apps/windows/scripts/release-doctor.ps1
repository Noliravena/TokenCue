...
<#
.SYNOPSIS
    Check whether a TokenCue release is ready or complete.
...
param(
    [string]$Version = "",
    [string]$AssetsDir = "C:\code\TokenCue-release\assets",
    [switch]$SkipGitHub
)
...
$tag = "v$Version"
Write-Host "Release doctor: TokenCue $Version"
Write-Host ""
...
if (Test-Path $AssetsDir) {
    Test-AssetHash (Join-Path $AssetsDir "TokenCue-$Version-Setup.exe")
    Test-AssetHash (Join-Path $AssetsDir "TokenCue-$Version-portable.exe")
} else {
...
if (-not $SkipGitHub) {
    ...
    if ($gh) {
        Push-Location $RepoRoot
        try {
            $ghJsonPath = Join-Path $env:TEMP "TokenCue-release-doctor-gh.json"
            $ghErrPath = Join-Path $env:TEMP "TokenCue-release-doctor-gh.err"
            $repoFlag = @()
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                foreach ($name in @(
                    "TokenCue-$Version-Setup.exe",
                    "TokenCue-$Version-Setup.exe.sha256",
                    "TokenCue-$Version-portable.exe",
                    "TokenCue-$Version-portable.exe.sha256"
                )) {
