...
if ($desktopHash -eq $cliHash) {
    throw "desktop and CLI executables must not be byte-identical; the CLI must be the console binary."
}
if ($desktopHash -ne $legacyDesktopHash) {
    throw "desktop and desktop-compatibility executables should be identical GUI binaries."
}
...
if ($desktopSubsystem -ne 2) {
    throw "desktop executable must be a Windows GUI-subsystem binary; got subsystem $desktopSubsystem."
}
if ($legacyDesktopSubsystem -ne 2) {
    throw "desktop-compatibility executable must be a Windows GUI-subsystem binary; got subsystem $legacyDesktopSubsystem."
}
if ($cliSubsystem -ne 3) {
    throw "CLI executable must be a Windows console-subsystem binary; got subsystem $cliSubsystem."
}

if ($CheckCliStdout) {
    $stdoutPath = Join-Path ([System.IO.Path]::GetTempPath()) "tokencue-cli-stdout-$PID.txt"
    try {
        & $cli --help > $stdoutPath
        ...
        if (-not $stdout -or $stdout -notmatch "Usage:" -or $stdout -notmatch "diagnose") {
            throw "CLI --help did not write expected CLI help to redirected stdout."
        }
    } finally {
