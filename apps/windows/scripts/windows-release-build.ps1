...
param(
    [string]$Ref = "HEAD",
    [string]$RepoUrl = "https://github.com/Noliravena/TokenCue.git",
    [string]$WorkRoot = "C:\code\TokenCue-release",
...
try {
    ...
    Write-Host "Building TokenCue $version from $commit"
    ...
    $sourceExe = Join-Path $releaseBinDir "tokencue-desktop.exe"
    ...
    $desktopExe = Join-Path $releaseBinDir "tokencue-desktop.exe"
    $legacyDesktopExe = $desktopExe
    $releaseExe = Join-Path $releaseBinDir "tokencue.exe"
    if (-not (Test-Path $sourceExe)) {
        throw "Missing expected Tauri binary: $sourceExe"
    }

    if ($sourceExe -ne $desktopExe) {
        Copy-Item $sourceExe $desktopExe -Force
    }
    if (Get-ObjdumpImportsWebView2Loader -ExePath $desktopExe) {
        throw "tokencue-desktop.exe imports WebView2Loader.dll, but release builds are expected to statically link the loader."
    }
    ...
    $sourceCliExe = Join-Path $cliBinDir "tokencue.exe"
    ...
    if ($WarmCacheOnly) {
        $warmExe = Join-Path $AssetsDir "TokenCue-$version-warm.exe"
    ...
    try {
        Invoke-Native $iscc @(
            ...
            "/DOutputBaseFilename=TokenCue-$version-Setup",
            "tokencue.iss"
        )
    } finally {
        Pop-Location
    ...
    $installer = Join-Path $installerOut "TokenCue-$version-Setup.exe"
    $portableExe = Join-Path $AssetsDir "TokenCue-$version-portable.exe"
    $installerAsset = Join-Path $AssetsDir "TokenCue-$version-Setup.exe"
    ...
    Get-ChildItem $AssetsDir -Filter "TokenCue-$version-*" |
        Sort-Object Name |
