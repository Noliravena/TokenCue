(function applyProviderPluginPrelude(ctx, host) {
  ...
  const classifiedFailure = kind => message =>
    new Error(`__TOKENCUE_FAILURE__:${kind}:${String(message)}`);
  ...
  ctx.date = Object.freeze({
    ...
    nextDailyReset(timeZone, hour) {
      ...
      if (typeof host.nextDailyReset === "function") {
        return new Date(Number(host.nextDailyReset(String(timeZone), resetHour)));
      }
      const now = new Date();
